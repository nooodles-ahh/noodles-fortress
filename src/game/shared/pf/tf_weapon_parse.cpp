...
CTFWeaponInfo::CTFWeaponInfo()
{
	...
	m_szMuzzleFlashViewModel[0] = '\0';
	m_flMuzzleFlashModelDuration = 0;
	m_szMuzzleFlashParticleEffect[0] = '\0';
	m_szMuzzleFlashParticleEffectViewModel[0] = '\0';
	m_flMuzzleFlashModelScale = 1.0f;
	...
	//m_pClassViewModelKV = nullptr;

	for (int i = 0; i < TF_CLASS_COUNT_ALL; i++)
	{
		m_szClassViewModel[i][0] = '\0';
	}
}
...
void CTFWeaponInfo::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_nHeal			= pKeyValuesData->GetInt("Heal", 0);
	...
	m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_flProjectileGravity	= pKeyValuesData->GetFloat( "ProjectileGravity", 0.0f );
	...
	m_WeaponData[TF_WEAPON_SECONDARY_MODE].m_iMaxAmmo			= pKeyValuesData->GetInt("Secondary_MaxAmmo", 0);
	...
	m_WeaponData[TF_WEAPON_SECONDARY_MODE].m_flProjectileGravity	= pKeyValuesData->GetFloat( "Secondary_ProjectileGravity", m_WeaponData[TF_WEAPON_PRIMARY_MODE].m_flProjectileGravity );
	...
	{
		m_iWeaponType = TF_WPN_TYPE_SCOUTSHOTGUN;
	}*/
	...
	else if (!Q_strcmp(pszWeaponType, "item3"))
	{
		m_iWeaponType = TF_WPN_TYPE_ITEM3;
	}
	else if (!Q_strcmp(pszWeaponType, "item4"))
	{
		m_iWeaponType = TF_WPN_TYPE_ITEM4;
	}
	...
	for (int i = TF_CLASS_SCOUT; i < TF_CLASS_COUNT_ALL; i++)
	{
		...
		/*else if (!Q_strcmp(pszWeaponType, "scoutshotgun"))
		{
			m_iClassWeaponType[i] = TF_WPN_TYPE_SCOUTSHOTGUN;
		}*/
		else if (!Q_strcmp(pszWeaponType, "item1"))
		{
			m_iClassWeaponType[i] = TF_WPN_TYPE_ITEM1;
		}
		else if (!Q_strcmp(pszWeaponType, "item2"))
		{
			m_iClassWeaponType[i] = TF_WPN_TYPE_ITEM2;
		}
		else if (!Q_strcmp(pszWeaponType, "item3"))
		{
			m_iClassWeaponType[i] = TF_WPN_TYPE_ITEM3;
		}
		else if (!Q_strcmp(pszWeaponType, "item4"))
		{
			m_iClassWeaponType[i] = TF_WPN_TYPE_ITEM4;
		}

	}

	m_flArmorPenetration = pKeyValuesData->GetFloat("Armor_Penetration_Amount", 0.0f);
	...
	m_bCanThrowGrenade		= (pKeyValuesData->GetInt( "CanThrowGrenade", 1 ) != 0);
	...
	// use the 3rd person muzzle model if we don't have a 1st person one
	const char *pszMuzzleFlashViewModel = pKeyValuesData->GetString( "MuzzleFlashViewmodel", NULL );
	if ( pszMuzzleFlashViewModel )
	{
		Q_strncpy( m_szMuzzleFlashViewModel, pszMuzzleFlashViewModel, sizeof( m_szMuzzleFlashViewModel ) );
	}
	else if( pszMuzzleFlashModel )
	{
		Q_strncpy( m_szMuzzleFlashViewModel, pszMuzzleFlashModel, sizeof( m_szMuzzleFlashViewModel ) );
	}

	m_flMuzzleFlashModelDuration = pKeyValuesData->GetFloat( "MuzzleFlashModelDuration", 0.2 );
	m_flMuzzleFlashModelScale = pKeyValuesData->GetFloat( "MuzzleFlashModelScale", 1.0f );
	...
	const char* pszMuzzleFlashParticleEffectViewModel = pKeyValuesData->GetString("MuzzleFlashParticleEffectViewModel", m_szMuzzleFlashParticleEffect);

	if (pszMuzzleFlashParticleEffectViewModel)
	{
		Q_strncpy(m_szMuzzleFlashParticleEffectViewModel, pszMuzzleFlashParticleEffectViewModel, sizeof(m_szMuzzleFlashParticleEffectViewModel));
	}
	...
	//DevMsg("Start finding Class-specific Viewmodels\n");
	for (int i = 0; i < TF_CLASS_COUNT_ALL; i++)
	{
		Q_strncpy(m_szClassViewModel[i],
#ifdef GAME_DLL
			pKeyValuesData->GetString(UTIL_VarArgs("%s_viewmodel", g_aPlayerClassNames_NonLocalized[i])),
#else
			pKeyValuesData->GetString(VarArgs("%s_viewmodel", g_aPlayerClassNames_NonLocalized[i])),
#endif
			MAX_WEAPON_STRING);
	}
...
{
	return m_WeaponData[iWeapon].m_nDamage;
}
