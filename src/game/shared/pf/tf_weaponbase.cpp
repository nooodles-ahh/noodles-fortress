...
#ifdef PF2
	CTraceFilterIgnoreTeammates filter( pEntity, COLLISION_GROUP_NONE, pEntity->GetTeamNumber() );
...
CTFWeaponBase::CTFWeaponBase()
{
	...
	m_iCurrentSeed = -1;
#ifdef PF2_CLIENT
	m_iMuzzleModelAttachment = -2; // set to unchecked
#endif
}
...
#ifdef PF2
	m_nLoadoutInfo = GetTFInventory()->FindWeapon( GetClassname() );
#endif
...
void CTFWeaponBase::Precache()
{
...
#ifdef PF2
	if ( GetMuzzleFlashModel_1st() )
	{
		PrecacheModel( GetMuzzleFlashModel_1st() );
	}
#endif
...
#ifdef PF2
	//for the viewmodel muzzleflashes
	if (pTFInfo->m_szMuzzleFlashParticleEffectViewModel && pTFInfo->m_szMuzzleFlashParticleEffectViewModel[0])
	{
		PrecacheParticleSystem(pTFInfo->m_szMuzzleFlashParticleEffectViewModel);
	}
#endif
...
#ifdef PF2
	//DevMsg("Start precaching Class-specific Viewmodels\n");
	...
	for (int i = 0; i < TF_CLASS_COUNT_ALL; i++)
	{
		...
		//DevMsg("The mdl file for class %s's %s being precached is: %s\n", g_aPlayerClassNames_NonLocalized[i], pTFInfo->szPrintName, pTFInfo->m_szClassViewModel[i]);
	}

	//DevMsg("The overall-mdl file being precached is: %s\n", pTFInfo->szViewModel);
#endif
}
// -----------------------------------------------------------------------------
// Purpose:
...
// Purpose:
// -----------------------------------------------------------------------------
 PFLoadoutWeaponInfo *CTFWeaponBase::GetPFItemInfo()
{
	PFLoadoutWeaponInfo *pPFInfo = GetTFInventory()->GetWeapon( m_nLoadoutInfo );
	Assert( pPFInfo );
	return pPFInfo;
}
#endif
...
bool CTFWeaponBase::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	...
	for (int i = 0; i < 2; i++)
	{
		if (m_hMuzzleFlashModel[i])
		{
			m_hMuzzleFlashModel[i]->SetParent(NULL, 0);
			m_hMuzzleFlashModel[i]->SetAbsOrigin(vec3_origin);
			m_hMuzzleFlashModel[i]->SetLifetime(0.0f);
		}
	}
#endif
	AbortReload();
...
void CTFWeaponBase::CalcIsAttackCritical( void)
{
	...
	if  ( ( TFGameRules()->State_Get() == GR_STATE_TEAM_WIN ) 
		...
		|| pPlayer->m_Shared.InCond( TF_COND_CRITBOOSTED ) )
	{
...
ConVar pf_allow_spy_fake_reload( "pf_allow_spy_fake_reload", "0", FCVAR_REPLICATED | FCVAR_NOTIFY );
//-----------------------------------------------------------------------------
// Purpose: 
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CTFWeaponBase::Reload( void )
{
	...
	// Don't reload if we're under an invis condition
	if (GetTFPlayerOwner()->m_Shared.InCond(TF_COND_STEALTHED) || 
		GetTFPlayerOwner()->m_Shared.InCond(TF_COND_SMOKE_BOMB) ||
		GetTFPlayerOwner()->m_Shared.InCond( TF_COND_BUILDING_DETPACK ) )
		return false;
	...
	{
		if( pf_allow_spy_fake_reload.GetBool() && GetTFPlayerOwner()->IsPlayerClass( TF_CLASS_SPY ) && GetTFPlayerOwner()->m_Shared.InCond( TF_COND_DISGUISED ) )
		{
...
bool CTFWeaponBase::DefaultReload( int iClipSize1, int iClipSize2, int iActivity )
{
	...
	bool bAllowFakeReload = pf_allow_spy_fake_reload.GetBool() && pPlayer->IsPlayerClass( TF_CLASS_SPY ) && pPlayer->m_Shared.InCond( TF_COND_DISGUISED );

	// We didn't reload.
	if ( !( bReloadPrimary || bReloadSecondary ) && !bAllowFakeReload )
		return false;
...
void CTFWeaponBase::ItemBusyFrame( void )
{
	...
	// if we can't attack we don't interrupt a reload
	if( !CanAttack() )
		return;
	...
	if (pPlayer)
	{
		// incase the CanAttack check causes problems
		//if( pPlayer->m_Shared.GetStealthNoAttackExpireTime() > gpGlobals->curtime )
		//	return;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTFWeaponBase::IsLowered(void)
{
	return m_bLowered;
}
...
{
	PFLoadoutWeaponInfo *info = GetPFItemInfo();

	FOR_EACH_VEC( info->m_ActivityOverrides, i )
	{
		// need the reference so we can actually update it
		override_acttable_t *act = &info->m_ActivityOverrides[i];
		if( act->acttable.baseAct == -2 )
		{
			act->acttable.baseAct = ActivityList_IndexForName( act->name );
			act->acttable.weaponAct = ActivityList_IndexForName( act->replace );
		}
		if( baseAct == act->acttable.baseAct )
		{
			if( pRequired )
			{
				*pRequired = act->acttable.required;
			}
			return (Activity)act->acttable.weaponAct;
		}
	}
...
bool CTFWeaponBase::Ready(void)
{
	...
	int sequence = SelectWeightedSequence( ACT_VM_LOWERED_TO_IDLE );

	m_bLowered = false;
	SendWeaponAnim( ACT_VM_IDLE );
	
	CBaseViewModel *pModel = GetTFPlayerOwner()->GetViewModel( m_nViewModelIndex );
	if( pModel )
	{
		pModel->RemoveEffects( EF_NODRAW );
		pModel->SendViewModelMatchingSequence( sequence );
	}
...
bool CTFWeaponBase::Lower( void )
{
	...
	CBaseViewModel *pModel = GetTFPlayerOwner()->GetViewModel( m_nViewModelIndex );
	int sequence = SelectWeightedSequence( ACT_VM_IDLE_TO_LOWERED );
...
void CTFWeaponBase::WeaponIdle( void )
{
	...
	else
	{
		...
		if( GetActivity() == ACT_VM_IDLE_LOWERED && HasWeaponIdleTimeElapsed() )
		{
			SendWeaponAnim( ACT_VM_IDLE );
			m_flTimeWeaponIdle = gpGlobals->curtime + SequenceDuration();
		}
...
// Purpose:
// -----------------------------------------------------------------------------
const char *CTFWeaponBase::GetMuzzleFlashModel_1st( void )
{ 
	const char *pszModel = GetTFWpnData().m_szMuzzleFlashViewModel;
...
const char* CTFWeaponBase::GetMuzzleFlashParticleEffect_1st( void )
{
	const char* pszPEffect = GetTFWpnData().m_szMuzzleFlashParticleEffectViewModel;
...
void CTFWeaponBase::CreateMuzzleFlashEffects(C_BaseEntity* pAttachEnt, int nIndex)
{
	...
	const char* pszMuzzleFlashModel = NULL;
	
	// Pick the right muzzleflash (3rd / 1st person)
	if ( UsingViewModel() )
	{
		pszMuzzleFlashEffect = GetMuzzleFlashEffectName_1st();
		pszMuzzleFlashParticleEffect = GetMuzzleFlashParticleEffect_1st();
		pszMuzzleFlashModel = GetMuzzleFlashModel_1st();
	}
	else
	{
		pszMuzzleFlashEffect = GetMuzzleFlashEffectName_3rd();
		pszMuzzleFlashParticleEffect = GetMuzzleFlashParticleEffect();
		pszMuzzleFlashModel = GetMuzzleFlashModel();
	}

	bool bVMGroupMuzzle = false;
	int iMuzzleFlashAttachment = -1;

	if( pszMuzzleFlashModel && UsingViewModel() && pf_muzzleflash.GetBool() )
	{
		// if we haven't gotten the muzzle model attachment get it now
		if( m_iMuzzleModelAttachment == -2 )
			m_iMuzzleModelAttachment = pAttachEnt->LookupAttachment( "muzzle_model" );

		iMuzzleFlashAttachment = m_iMuzzleModelAttachment;
		bVMGroupMuzzle = true;
	}
	
	if( iMuzzleFlashAttachment <= 0 )
	{
		iMuzzleFlashAttachment = pAttachEnt->LookupAttachment( "muzzle" );
		bVMGroupMuzzle = false;
	}
	...
	if (iMuzzleFlashAttachment > 0 && (pszMuzzleFlashEffect || pszMuzzleFlashModel || pszMuzzleFlashParticleEffect))
	{
		...
		if (pf_muzzlelight.GetBool())
		{
			...
			TE_DynamicLight( filter, 0.0f, &vecOrigin, 255, 192, 64, 5, 70.0f, 0.08f, 70.0f / 0.05f, LIGHT_INDEX_MUZZLEFLASH + index );
		}
	}
	...
	if( pszMuzzleFlashModel && pf_muzzleflash.GetBool() )
	{
		...
		{
			m_hMuzzleFlashModel[nIndex] = C_MuzzleFlashModel::CreateMuzzleFlashModel(pszMuzzleFlashModel, pAttachEnt, 
				iMuzzleFlashAttachment, flEffectLifetime, GetMuzzleFlashModelScale(), bVMGroupMuzzle );
		}
	}

	if( pszMuzzleFlashParticleEffect && ( !pf_muzzleflash.GetBool() || pszMuzzleFlashModel == NULL ) )
	{
...
{
	C_BaseEntity *pAttachEnt = GetWeaponForEffect();
	C_TFPlayer *pOwner = ToTFPlayer( GetOwnerEntity() );
	...
	if ( bDrawMuzzleFlashOnViewModel && (!r_drawviewmodel.GetBool() || pOwner->m_Shared.InCond(TF_COND_ZOOMED)))
		return;
...
acttable_t CTFWeaponBase::m_acttableItem1[] =
{
	...
	{ ACT_MP_RELOAD_STAND, ACT_MP_RELOAD_STAND_ITEM1, false },
	{ ACT_MP_RELOAD_STAND_LOOP, ACT_MP_RELOAD_STAND_ITEM1_LOOP, false },
	{ ACT_MP_RELOAD_STAND_END, ACT_MP_RELOAD_STAND_ITEM1_END, false },
	{ ACT_MP_RELOAD_CROUCH, ACT_MP_RELOAD_CROUCH_ITEM1, false },
	{ ACT_MP_RELOAD_CROUCH_LOOP, ACT_MP_RELOAD_CROUCH_ITEM1_LOOP, false },
	{ ACT_MP_RELOAD_CROUCH_END, ACT_MP_RELOAD_CROUCH_ITEM1_END, false },
	{ ACT_MP_RELOAD_SWIM, ACT_MP_RELOAD_SWIM_ITEM1, false },
	{ ACT_MP_RELOAD_SWIM_LOOP, ACT_MP_RELOAD_SWIM_ITEM1_LOOP, false },
	{ ACT_MP_RELOAD_SWIM_END, ACT_MP_RELOAD_SWIM_ITEM1_END, false },
	{ ACT_MP_RELOAD_AIRWALK, ACT_MP_RELOAD_AIRWALK_ITEM1, false },
	{ ACT_MP_RELOAD_AIRWALK_LOOP, ACT_MP_RELOAD_AIRWALK_ITEM1_LOOP, false },
	{ ACT_MP_RELOAD_AIRWALK_END, ACT_MP_RELOAD_AIRWALK_ITEM1_END, false },
...
acttable_t CTFWeaponBase::m_acttableItem2[] =
{
	...
	{ ACT_MP_ATTACK_STAND_SECONDARYFIRE, ACT_MP_ATTACK_STAND_ITEM2, false },
	{ ACT_MP_ATTACK_CROUCH_PRIMARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM2, false },
	{ ACT_MP_ATTACK_CROUCH_SECONDARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM2, false },
...
acttable_t CTFWeaponBase::m_acttableItem3[] =
{
	...
	{ ACT_MP_ATTACK_STAND_SECONDARYFIRE, ACT_MP_ATTACK_STAND_ITEM3, false },
	{ ACT_MP_ATTACK_CROUCH_PRIMARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM3, false },
	{ ACT_MP_ATTACK_CROUCH_SECONDARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM3, false },
	...
	{ ACT_MP_RELOAD_STAND, ACT_MP_RELOAD_STAND_ITEM3, false },
	{ ACT_MP_RELOAD_STAND_LOOP, ACT_MP_RELOAD_STAND_ITEM3_LOOP, false },
	{ ACT_MP_RELOAD_STAND_END, ACT_MP_RELOAD_STAND_ITEM3_END, false },
	{ ACT_MP_RELOAD_CROUCH, ACT_MP_RELOAD_CROUCH_ITEM3, false },
	{ ACT_MP_RELOAD_CROUCH_LOOP, ACT_MP_RELOAD_CROUCH_ITEM3_LOOP, false },
	{ ACT_MP_RELOAD_CROUCH_END, ACT_MP_RELOAD_CROUCH_ITEM3_END, false },
	{ ACT_MP_RELOAD_SWIM, ACT_MP_RELOAD_SWIM_ITEM3, false },
	{ ACT_MP_RELOAD_SWIM_LOOP, ACT_MP_RELOAD_SWIM_ITEM3_LOOP, false },
	{ ACT_MP_RELOAD_SWIM_END, ACT_MP_RELOAD_SWIM_ITEM3_END, false },
	{ ACT_MP_RELOAD_AIRWALK, ACT_MP_RELOAD_AIRWALK_ITEM3, false },
	{ ACT_MP_RELOAD_AIRWALK_LOOP, ACT_MP_RELOAD_AIRWALK_ITEM3_LOOP, false },
	{ ACT_MP_RELOAD_AIRWALK_END, ACT_MP_RELOAD_AIRWALK_ITEM3_END, false },

	{ ACT_MP_GESTURE_FLINCH, ACT_MP_GESTURE_FLINCH_ITEM3, false },

	{ ACT_MP_GRENADE1_DRAW, ACT_MP_ITEM3_GRENADE1_DRAW, false },
	{ ACT_MP_GRENADE1_IDLE, ACT_MP_ITEM3_GRENADE1_IDLE, false },
	{ ACT_MP_GRENADE1_ATTACK, ACT_MP_ITEM3_GRENADE1_ATTACK, false },
	{ ACT_MP_GRENADE2_DRAW, ACT_MP_ITEM3_GRENADE2_DRAW, false },
	{ ACT_MP_GRENADE2_IDLE, ACT_MP_ITEM3_GRENADE2_IDLE, false },
	{ ACT_MP_GRENADE2_ATTACK, ACT_MP_ITEM3_GRENADE2_ATTACK, false },
	...
	{ ACT_MP_ATTACK_STAND_GRENADE, ACT_MP_ATTACK_STAND_GRENADE_ITEM3, false },
	{ ACT_MP_ATTACK_CROUCH_GRENADE, ACT_MP_ATTACK_CROUCH_GRENADE_ITEM3, false },
	{ ACT_MP_ATTACK_SWIM_GRENADE, ACT_MP_ATTACK_SWIM_GRENADE_ITEM3, false },
	{ ACT_MP_ATTACK_AIRWALK_GRENADE, ACT_MP_ATTACK_AIRWALK_GRENADE_ITEM3, false },

	{ ACT_MP_GESTURE_VC_HANDMOUTH, ACT_MP_GESTURE_VC_HANDMOUTH_ITEM3, false },
	{ ACT_MP_GESTURE_VC_FINGERPOINT, ACT_MP_GESTURE_VC_FINGERPOINT_ITEM3, false },
	{ ACT_MP_GESTURE_VC_FISTPUMP, ACT_MP_GESTURE_VC_FISTPUMP_ITEM3, false },
	{ ACT_MP_GESTURE_VC_THUMBSUP, ACT_MP_GESTURE_VC_THUMBSUP_ITEM3, false },
	{ ACT_MP_GESTURE_VC_NODYES, ACT_MP_GESTURE_VC_NODYES_ITEM3, false },
	{ ACT_MP_GESTURE_VC_NODNO, ACT_MP_GESTURE_VC_NODNO_ITEM3, false },
};

acttable_t CTFWeaponBase::m_acttableItem4[] =
{
	...
	{ ACT_MP_ATTACK_STAND_SECONDARYFIRE, ACT_MP_ATTACK_STAND_ITEM4, false },
	{ ACT_MP_ATTACK_CROUCH_PRIMARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM4, false },
	{ ACT_MP_ATTACK_CROUCH_SECONDARYFIRE, ACT_MP_ATTACK_CROUCH_ITEM4, false },
	...
	{ ACT_MP_RELOAD_STAND, ACT_MP_RELOAD_STAND_ITEM4, false },
	{ ACT_MP_RELOAD_STAND_LOOP, ACT_MP_RELOAD_STAND_ITEM4_LOOP, false },
	{ ACT_MP_RELOAD_STAND_END, ACT_MP_RELOAD_STAND_ITEM4_END, false },
	{ ACT_MP_RELOAD_CROUCH, ACT_MP_RELOAD_CROUCH_ITEM4, false },
	{ ACT_MP_RELOAD_CROUCH_LOOP, ACT_MP_RELOAD_CROUCH_ITEM4_LOOP, false },
	{ ACT_MP_RELOAD_CROUCH_END, ACT_MP_RELOAD_CROUCH_ITEM4_END, false },
	{ ACT_MP_RELOAD_SWIM, ACT_MP_RELOAD_SWIM_ITEM4, false },
	{ ACT_MP_RELOAD_SWIM_LOOP, ACT_MP_RELOAD_SWIM_ITEM4_LOOP, false },
	{ ACT_MP_RELOAD_SWIM_END, ACT_MP_RELOAD_SWIM_ITEM4_END, false },
	{ ACT_MP_RELOAD_AIRWALK, ACT_MP_RELOAD_AIRWALK_ITEM4, false },
	{ ACT_MP_RELOAD_AIRWALK_LOOP, ACT_MP_RELOAD_AIRWALK_ITEM4_LOOP, false },
	{ ACT_MP_RELOAD_AIRWALK_END, ACT_MP_RELOAD_AIRWALK_ITEM4_END, false },

	{ ACT_MP_GESTURE_FLINCH, ACT_MP_GESTURE_FLINCH_ITEM4, false },

	{ ACT_MP_GRENADE1_DRAW, ACT_MP_ITEM4_GRENADE1_DRAW, false },
	{ ACT_MP_GRENADE1_IDLE, ACT_MP_ITEM4_GRENADE1_IDLE, false },
	{ ACT_MP_GRENADE1_ATTACK, ACT_MP_ITEM4_GRENADE1_ATTACK, false },
	{ ACT_MP_GRENADE2_DRAW, ACT_MP_ITEM4_GRENADE2_DRAW, false },
	{ ACT_MP_GRENADE2_IDLE, ACT_MP_ITEM4_GRENADE2_IDLE, false },
	{ ACT_MP_GRENADE2_ATTACK, ACT_MP_ITEM4_GRENADE2_ATTACK, false },
	...
	{ ACT_MP_ATTACK_STAND_GRENADE, ACT_MP_ATTACK_STAND_GRENADE_ITEM4, false },
	{ ACT_MP_ATTACK_CROUCH_GRENADE, ACT_MP_ATTACK_CROUCH_GRENADE_ITEM4, false },
	{ ACT_MP_ATTACK_SWIM_GRENADE, ACT_MP_ATTACK_SWIM_GRENADE_ITEM4, false },
	{ ACT_MP_ATTACK_AIRWALK_GRENADE, ACT_MP_ATTACK_AIRWALK_GRENADE_ITEM4, false },

	{ ACT_MP_GESTURE_VC_HANDMOUTH, ACT_MP_GESTURE_VC_HANDMOUTH_ITEM4, false },
	{ ACT_MP_GESTURE_VC_FINGERPOINT, ACT_MP_GESTURE_VC_FINGERPOINT_ITEM4, false },
	{ ACT_MP_GESTURE_VC_FISTPUMP, ACT_MP_GESTURE_VC_FISTPUMP_ITEM4, false },
	{ ACT_MP_GESTURE_VC_THUMBSUP, ACT_MP_GESTURE_VC_THUMBSUP_ITEM4, false },
	{ ACT_MP_GESTURE_VC_NODYES, ACT_MP_GESTURE_VC_NODYES_ITEM4, false },
	{ ACT_MP_GESTURE_VC_NODNO, ACT_MP_GESTURE_VC_NODNO_ITEM4, false },
};
...
acttable_t* CTFWeaponBase::ActivityList(int& iActivityCount)
{
	...
	case TF_WPN_TYPE_ITEM4:
		...
		iActivityCount = ARRAYSIZE(m_acttableItem4);
		break;
		/*case TF_WPN_TYPE_BUILDINGARM:
			for (int i = 0; i < 2; i++)
			{
...
// Purpose:
// -----------------------------------------------------------------------------
CBasePlayer *CTFWeaponBase::GetPlayerOwner() const // moved to header
{
...
#if defined( CLIENT_DLL )
#ifdef PF2_CLIENT
static ConVar	cl_bobcycle( "cl_bobcycle","0.8", FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
static ConVar	cl_bobup( "cl_bobup","0.5", FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
static ConVar	cl_bob("cl_bob", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
#else
...
int CTFWeaponBase::GetSkin()
{
	...
	if (pPlayer)
	{
		nSkin = pPlayer->GetTeamNumber();
	}

	if (pPlayer && pPlayer->m_Shared.InCond(TF_COND_DISGUISED) && pPlayer->IsEnemyPlayer())
	{
		nSkin = pPlayer->m_Shared.GetDisguiseTeam();
	}

	return GetTeamSkin( nSkin );
}
