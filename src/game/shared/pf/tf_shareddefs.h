...
#ifdef PF2
extern const char* g_aTeamNamesLower[ TF_TEAM_COUNT ];
extern const char *g_aTeamNamesShort[ TF_TEAM_COUNT ];
#endif

const char *GetTeamParticleName( int iTeam, const char **pNames = g_aTeamParticleNames );
const char *ConstructTeamParticle( const char *pszFormat, int iTeam, const char **pNames = g_aTeamParticleNames );
void PrecacheTeamParticles( const char *pszFormat, const char **pNames = g_aTeamParticleNames );
int GetTeamSkin( int iTeam );
...
#ifdef PF2
#define TF_VM_WEAPON 0
#define TF_VM_OFFHAND 1

// Cyanide; A marco so I can see when something was hit on the client and server
#ifdef DEBUG
#ifdef CLIENT_DLL
#define MSG_NOTIFY_ON_HIT Msg( "Client hit %s at %f\n", __func__, gpGlobals->curtime )
#else
#define MSG_NOTIFY_ON_HIT Warning( "Server hit %s at %f\n", __func__, gpGlobals->curtime )
#endif
#else
#define MSG_NOTIFY_ON_HIT NULL
#endif
...
{
	PRIME_STATE_NONE = 0,
	PRIME_STATE_PRE_DEPLOY,
	PRIME_STATE_DEPLOYED,
};

extern const char* g_aArmorTypesLower[ ARMOR_COUNT ];

enum eFriendlyFireMode
{
	FFMODE_OFF = 0,
	FFMODE_EVENTS,
	FFMODE_ON,
...
#ifdef CLIENT_DLL
//Returns the value held inside the mapname key in the "scripts/discord_rpc.txt"
//-Nbc66
const char* GetRPCMapImage(char m_szLatchedMapname[MAX_MAP_NAME]);
const char* GetRPCMapName(char m_szLatchedMapname[MAX_MAP_NAME]);

// Cyanide; If we want to have 4 different overridable colours we have to write out "REGISTER_COLOR..." 4 times, every time. So here's a marco that does just that
#define REGISTER_OVERRIDABLE_TEAM_COLORS( colorArray, strug ) \
	REGISTER_COLOR_AS_OVERRIDABLE( colorArray[TEAM_UNASSIGNED], strug "0"); \
	REGISTER_COLOR_AS_OVERRIDABLE( colorArray[TEAM_SPECTATOR], strug "1" ); \
	REGISTER_COLOR_AS_OVERRIDABLE( colorArray[TF_TEAM_RED], strug "2" ); \
	REGISTER_COLOR_AS_OVERRIDABLE( colorArray[TF_TEAM_BLUE], strug "3")
#endif
#endif
...
#ifdef PF2
#define TF_LAST_PLAYABLE_CLASS	( TF_CLASS_CIVILIAN )
#endif
...
extern const char *g_aPlayerClassNamesLower[];				// localized class names
...
enum eFlagtypes
{
	...
	TF_FLAGTYPE_INVADE,
	TF_FLAGTYPE_KINGOFTHEHILL, // replaced with special delivery in live
	TF_FLAGTYPE_RD, // robot destruction place holder
};
...
#ifdef PF2
#define TF_TRANQUILLIZED_DURATION						6.0f
#define TF_CONCUSSION_DURATION							5.0f
#define TF_HALLUCINATION_RADIUS							256
#define TF_HALLUCINATION_TIME							12.0f
#else
#define TF_WEAPON_GRENADE_CONCUSSION_TIME				15.0f
#endif

#define TF_WEAPON_GRENADE_MIRV_BOMB_COUNT				4
#define TF_BROKEN_LEG_DURATION							6.0f
#define TF_BROKEN_LEG_GUNSHOT_DURATION					6.0f
...
enum eTFWeapons
{
	...
	TF_WEAPON_SHOTGUN_DOUBLEBARREL,
#endif
	// ADD WEAPONS HERE OR ELSE DEMOS WILL BREAK

	TF_WEAPON_COUNT
};
extern const Vector g_vecFixedWpnSpreadPellets[];
#ifdef PF2
extern const Vector g_vecCaltropFixedPattern[];
#endif
...
#ifdef PF2
// Infection
#define TF_INFECTION_FREQUENCY		2.0f
#define TF_INFECTION_DMG			6
#endif
...
enum eTFConds
{
	...
	TF_COND_ARMOR_MELTING,
	TF_COND_LEG_DAMAGED_GUNSHOT,
	TF_COND_ARMOR_BUFF,
#endif
...
// TF FlagInfo State.
//-----------------------------------------------------------------------------
#define TF_FLAGINFO_NONE		(1<<0)
#define TF_FLAGINFO_STOLEN		(1<<1)
#define TF_FLAGINFO_DROPPED		(1<<2)
#define TF_FLAGINFO_THROWN		(1<<3)
...
#ifdef PF2
#define DMG_INFECTION			(DMG_PHYSGUN)
#endif
...
#ifdef PF2
// Turret info
#define TURRET_EYE_OFFSET			Vector( 0, 0, -24 )
#define TURRET_GROUND_EYE_OFFSET	Vector( 0, 0, 6 )
#endif
...
typedef enum
{
	...
	BUILDING_HUD_ALERT_ENEMY_USING,
...
#ifdef PF2
extern float g_flDispenserArmorRates[][ARMOR_COUNT];
#endif
...
enum eTurretStates
{
	TURRET_STATE_INACTIVE = 0,	
	TURRET_STATE_DORMANT,		// The main state of the spawnroom turret. It would stay in this state most of the time.
	TURRET_STATE_DEPLOY,
	TURRET_STATE_RETRACT,
	TURRET_STATE_SEARCHING,		// It would go to this state shortly after detecting an enemy.
	TURRET_STATE_ATTACKING,		// Attacking.
...
#define OBJECT_PICKUP_DISTANCE 150.0f
...
public:
	...
	float	m_flUpgradeDuration;			// The time it takes for a building to upgrade
	...
	const char*	m_pUpgradeSound;					// gamesound to play when upgrading
	int		m_BuildCount;					// ???
...
typedef enum
{
	ENGICALL_DEFAULT = 0x00,
	ENGICALL_MELTING = 0x01,
	ENGICALL_LOWARMOR = 0x02
} EngineerCallType_t;
...
public:
	...
	{
		bool ret = BaseClass::ShouldHitEntity( pServerEntity, contentsMask );
		if( ret )
		{
