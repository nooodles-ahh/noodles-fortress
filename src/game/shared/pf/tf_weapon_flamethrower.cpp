...
#ifdef PF2
	ConVar	tf_flamethrower_speed_penalty_enabled( "tf_flamethrower_speed_penalty_enabled", "0", FCVAR_REPLICATED | FCVAR_NOTIFY, "Enable attack movement speed penalty." );
	ConVar	tf_flamethrower_speed_penalty( "tf_flamethrower_speed_penalty", "200", FCVAR_REPLICATED | FCVAR_NOTIFY, "Attack movement speed penalty speed." );
#endif
...
BEGIN_NETWORK_TABLE( CTFFlameThrower, DT_WeaponFlameThrower )
		...
		RecvPropBool( RECVINFO( m_bTargetHit ) ),
	#else
		SendPropInt( SENDINFO( m_iWeaponState ), 4, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN ),
		SendPropBool( SENDINFO( m_bCritFire ) ),
		SendPropBool( SENDINFO( m_bTargetHit ) ),
	#endif
END_NETWORK_TABLE()
...
bool CTFFlameThrower::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	...
	if ( pOwner )
	{
		if ( m_iWeaponState < FT_STATE_STARTFIRING )
		{
...
void CTFFlameThrower::ItemPostFrame()
{
	...
	else if ( m_iWeaponState < FT_STATE_STARTFIRING )
	{
		if ( pOwner->m_Shared.InCond( TF_COND_AIMING ) )
		{
			pOwner->m_Shared.RemoveCond( TF_COND_AIMING );
...
void CTFFlameThrower::PrimaryAttack()
{
	...
	if ( tf_flamethrower_speed_penalty_enabled.GetBool() )
	{
...
#ifdef CLIENT_DLL
	// If we have a host but the host is not the previous host, restart the effect
	if( m_hFlameEffectHost && m_hFlameEffectHost != GetWeaponForEffect() )
	{
	...
	if (pf_muzzlelight.GetBool() && pOwner->GetWaterLevel() != WL_Eyes)
	{
		...
		dl->decay = 512.0f;
		dl->style = 1;
		dl->die = gpGlobals->curtime + 0.04f;
	}
#endif
	...
	if ( pOwner->GetWaterLevel() != WL_Eyes )
	{
		...
		int iDamagePerSec = m_pWeaponInfo->GetWeaponDamage( m_iWeaponMode );
	...
	m_flAmmoUseRemainder += m_pWeaponInfo->GetWeaponData(m_iWeaponMode).m_iAmmoPerShot * flFiringInterval;
	...
	if (!pf_infinite_ammo.GetBool() && iAmmoToSubtract > 0 )
	{

		if (UsesClipsForAmmo1())
		{
			m_iClip1 -= iAmmoToSubtract;
...
void CTFFlameThrower::SecondaryAttack()
{
	...
	{
#ifdef CLIENT_DLL
		if (flNextFailAttack < gpGlobals->curtime)
		{
			flNextFailAttack = gpGlobals->curtime + 0.5f;
	...
	int iAmmoConsumption = m_pWeaponInfo->GetWeaponData(m_iWeaponMode).m_iAmmoPerShot;

	if(!pf_infinite_ammo.GetBool())
	{
		if (UsesClipsForAmmo1())
		{
			if (m_iClip1 >= iAmmoConsumption)
			{
				m_iClip1 -= iAmmoConsumption;
			}
			else
			{
				return;
		...
		{
			if (pPlayer->GetAmmoCount(m_iPrimaryAmmoType) >= iAmmoConsumption)
			{
				pPlayer->RemoveAmmo(iAmmoConsumption, m_iPrimaryAmmoType);
			}
			else
			{
				return;
	...
	FireProjectile(pPlayer, false);

	m_bInAttack2 = true;
	// Set next attack times.
	m_iWeaponState = FT_STATE_IDLE;
	m_flNextPrimaryAttack = gpGlobals->curtime + 1.1; //stop flaming until done firing rocket
...
//
//-----------------------------------------------------------------------------
bool CTFFlameThrower::Reload(void)
{
...
void CTFFlameThrower::OnDataChanged(DataUpdateType_t updateType)
{
	...
	if (m_iWeaponState > FT_STATE_IDLE)
	{
		...
		{
			const char *pilotlightsound = "Weapon_FlameThrower.FireHit";
			CLocalPlayerFilter filter;

			CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
			m_pFlameHitSound = controller.SoundCreate( filter, entindex(), pilotlightsound );

			controller.Play( m_pFlameHitSound, 1.0, 100 );
		}
		return;
	}
	else
...
{
	if( m_pFlameHitSound ) {
		CSoundEnvelopeController::GetController().SoundDestroy( m_pFlameHitSound );
		m_pFlameHitSound = NULL;
	}
	m_bTargetHit = false;
}
#else // GAME_DLL
void CTFFlameThrower::SetHitTarget()
{
	if( m_iWeaponState < FT_STATE_STARTFIRING )
		return;
	...
	m_flNextFlameHitThink = gpGlobals->curtime + 0.2;

	// Setup the think function.
	SetThink( &CTFFlameThrower::HitTargetThink );
	SetNextThink( gpGlobals->curtime );
}

void CTFFlameThrower::HitTargetThink(  )
{

	if( (m_flNextFlameHitThink <= 0.0) || (gpGlobals->curtime <= m_flNextFlameHitThink) )
	{
...
void CTFFlameEntity::FlameThink( void )
{
	...
	if ( GetAbsOrigin() != m_vecPrevPos )
	{
		...
		pAttacker->GetOpposingTFTeamList( &pTeamList, TFGameRules()->FriendlyFireMode() );
...
void CTFFlameEntity::SetHitTarget()
{
	if( !m_hAttacker.Get() )
		return;

	if( !m_hAttacker.Get()->IsPlayer() )
		return;

	CTFPlayer *pTFPlayer = ToTFPlayer( m_hAttacker.Get() );
	if( pTFPlayer )
	{
		CTFWeaponBase *pWeapon = pTFPlayer->GetActiveTFWeapon();
		if( pWeapon && pWeapon->GetWeaponID() == TF_WEAPON_FLAMETHROWER )
		{
			CTFFlameThrower *pFlameThrower = dynamic_cast<CTFFlameThrower *>(pWeapon);
			if( pFlameThrower )
				pFlameThrower->SetHitTarget();
		}
	}
