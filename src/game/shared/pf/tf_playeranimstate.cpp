...
Activity CTFPlayerAnimState::TranslateActivity( Activity actDesired )
{
	...
	if ( GetTFPlayer()->m_Shared.IsCarryingObject() )
	{
...
bool CTFPlayerAnimState::HandleMoving( Activity &idealActivity )
{
	...
	if ( (m_pTFPlayer->m_Shared.InCond( TF_COND_AIMING ) && !m_pTFPlayer->IsPlayerClass( TF_CLASS_SNIPER ) ) || 
		m_pTFPlayer->m_Shared.InCond( TF_COND_ZOOMED ) )
	{
		if ( flSpeed > MOVING_MINIMUM_SPEED )
		{
...
bool CTFPlayerAnimState::HandleDucking( Activity &idealActivity )
{
	if ( GetBasePlayer()->GetFlags() & FL_DUCKING )
	{
		if ( GetOuterXYSpeed() < MOVING_MINIMUM_SPEED || m_pTFPlayer->m_Shared.IsLoser() )
		{
			...
			if ( ( m_pTFPlayer->m_Shared.InCond( TF_COND_AIMING ) && m_pTFPlayer->IsPlayerClass( TF_CLASS_HEAVYWEAPONS ) ) ||
				m_pTFPlayer->m_Shared.InCond( TF_COND_ZOOMED ) ||
				m_flHoldDeployedPoseUntilTime > gpGlobals->curtime )
			{
		...
		else
		{
			...
			if ( m_pTFPlayer->m_Shared.InCond( TF_COND_ZOOMED ))
			{
				idealActivity = ACT_MP_CROUCHWALK_DEPLOYED;
			}
		}
