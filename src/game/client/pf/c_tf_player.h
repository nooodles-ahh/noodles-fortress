...
extern ConVar pf_holdzoom;
extern ConVar cl_autoreload;
extern ConVar pf_grenade_press_throw;
extern ConVar pf_sniper_toggle_charge;
extern ConVar cl_flipviewmodels;
enum
{
	TF_HALLUCINATION_EXPLOSION,
	TF_HALLUCINATION_ONFIRE,
	TF_HALLUCINATION_EXTINGUISHFIRE,
	TF_HALLUCINATION_SHOTGUNSHOT,
	TF_HALLUCINATION_MELEEHIT_WORLD,
	TF_HALLUCINATION_MELEEHIT_FLESH,
	TF_HALLUCINATION_DEATHSCREAM,
	TF_HALLUCINATION_GRENADE,
};
...
public:
	...
	bool			IsActiveTFWeapon(int iWeapon);

	virtual void Simulate( void );
	virtual void FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options );
	void	DoHallucination();
	...
	void CreateConcussionEffect( void );
	void CreateSleepyEffect( void );
	void CreatePoisonEffect( void );
	void CreateArmorMeltEffect( void );
	void CreateInfectionEffect( void );

	void DoPlayerStatusAlerts( void );
	...
	bool			ShouldHoldZoom( void ){ return pf_holdzoom.GetBool(); }
	bool			ShouldAutoReload( void ){ return cl_autoreload.GetBool(); }
	bool			ShouldToggleSniperCharge( void ){ return pf_sniper_toggle_charge.GetBool(); }
	bool			IsViewModelFlipped( void ){ return cl_flipviewmodels.GetBool(); }
...
private:
	...
	*/
	...
	// Concusssion stars particle effect
	CNewParticleEffect	*m_pConcStarsEffect;

	// Sleep zees particle effect
	CNewParticleEffect	*m_pSleepyEffect;

	// Poison skulls particle effect
	CNewParticleEffect *m_pPoisonEffect;

	CNewParticleEffect	*m_pArmorMeltEffect;

	// Infection particle effect
	CNewParticleEffect	*m_pInfectionEffects[2];

	bool m_bPlayedLowArmorAlert;
	bool m_bPlayedDepletedArmorAlert;
	bool m_bPlayedLowHealthAlert;
