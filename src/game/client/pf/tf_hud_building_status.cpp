	...
	BaseClass( parent, "BuildingStatusItem" )
{
	...
	m_pEnemyUsingIcon = new CIconPanel( m_pBuiltPanel, "EnemyUsingIcon" );
...
void CBuildingStatusItem::PerformLayout( void )
{
	...
	else
	{
		...
		if ( m_pAlertTray->IsTrayOut() )
		{
			...
			m_pEnemyUsingIcon->SetVisible( false );
		}
	}
}
...
void CBuildingStatusItem::OnTick()
{
	...
	else
	{
			...
			m_pAlertTray->GetPercentDeployed() >= 1.0f )
		{
			...
			{
			case BUILDING_HUD_ALERT_ENEMY_USING:
				m_pEnemyUsingIcon->SetVisible(true);
				break;
...
DECLARE_HUDELEMENT( CHudBuildingStatusContainer_Demoman );

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CHudBuildingStatusContainer_Demoman::CHudBuildingStatusContainer_Demoman( const char *pElementName ) :
BaseClass( "BuildingStatus_Demoman" )
{
	AddBuildingPanel( OBJ_DETPACK, OBJECT_MODE_NONE );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
bool CHudBuildingStatusContainer_Demoman::ShouldDraw( void )
{
	// Don't draw at all as we don't have the detpack enabled
	return false;

	// Don't draw in freezecam
	C_TFPlayer *pPlayer = CTFPlayer::GetLocalTFPlayer();
	if (!pPlayer || !pPlayer->IsPlayerClass( TF_CLASS_DEMOMAN ) || pPlayer->GetObserverMode() == OBS_MODE_FREEZECAM)
	{
		return false;
	}
...
void CHudBuildingStatusContainer::OnBuildingChanged( int iBuildingType, int iObjectMode )
{
	...
	for ( int i = 0; i < m_BuildingPanels.Count() && !bFound; i++ )
	{
		...
		if ( pItem && pItem->GetRepresentativeObjectType() == iBuildingType && 
			pItem->GetRepresentativeObjectMode() == iObjectMode )
		{
			// find the item that represents this building type
			C_BaseObject *pObj = NULL;
...
void CHudBuildingStatusContainer::RecalculateAlertState( void )
{
	...
	{
		if (maxAlertLevel == BUILDING_HUD_ALERT_ENEMY_USING)
		{
			m_flNextBeep = 0;
			m_iNumBeepsToBeep = 0;
		}
		else if ( maxAlertLevel >= BUILDING_HUD_ALERT_VERY_LOW_AMMO)
		{
