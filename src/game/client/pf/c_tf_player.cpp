...
CON_COMMAND( dump_debug_spec_info, "Dump all potentially helpful information in debugging the invisible players when spectating bug" )
{
	...
	Q_snprintf( filename, filenamelen, "%s\\spec_debug_%d.txt", engine->GetGameDirectory(), steamapicontext->SteamUtils()->GetServerRealTime() );
	Msg( "writing shit to %s\n", filename );
	...
	Vector org = pLocalPlayer->GetAbsOrigin();
	Q_snprintf( writebuf, strbuflen, "player count: %d, current team: %d, coords: %f %f %f\n", total, pLocalPlayer->GetTeamNumber(), org.x, org.y, org.z );
	...
	for( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if( g_PR && g_PR->IsConnected( i ) )
		{
			...
			// player name
			Q_snprintf( writebuf, strbuflen, "%s:", pPlayer->GetPlayerName());
			filesystem->Write( writebuf, strlen( writebuf ), hFile );

			// player id
			Q_snprintf( writebuf, strbuflen, "%d, ", pPlayer->GetUserID() );
			filesystem->Write( writebuf, strlen( writebuf ), hFile );

			// in spec target?
			if( pPlayer == pLocalPlayer->GetObserverTarget() )
				filesystem->Write( "target, ", strlen( "target, " ), hFile );
			else
				filesystem->Write( "not target, ", strlen( "not target, " ), hFile );

			// ef nodraw
			if( pPlayer->GetEffects() & EF_NODRAW )
				filesystem->Write( "nodrawed, ", strlen( "nodrawed, " ), hFile );
			else
				filesystem->Write( "drawed, ", strlen( "drawed, " ), hFile );

			// ef nodraw
			if( pPlayer->IsAlive() )
				filesystem->Write( "alive, ", strlen( "alive, " ), hFile );
			else
				filesystem->Write( "dead, ", strlen( "dead, " ), hFile );

			// coords
			Vector org = pPlayer->GetAbsOrigin();
			Q_snprintf( writebuf, strbuflen, "coords: %f %f %f", org.x, org.y, org.z );
			filesystem->Write( writebuf, strlen( writebuf ), hFile );
...
#define INFECTION_PART_HEAD 0
#define INFECTION_PART_BODY 1
#endif
...
#ifdef DEBUG
ConVar pf_hallucination_effect( "pf_hallucination_effect", "0", FCVAR_ARCHIVE | FCVAR_NOT_CONNECTED, "Turn on/off the halucination effect");
#endif // DEBUG
...
void C_TFRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
	...
	else
	{
		...
		// Blood spray!
		FX_TF_BloodSpray( vecHitPos, vecDir, 10 );
	}
...
void C_TFRagdoll::CreateTFRagdoll()
{
	...
	// DIEVIOLENT ANIMATIONS PLAY ALL THE TIME
	if ( pPlayer && m_bOnGround && ( tf_always_deathanim.GetInt() > 0 || (tf_always_deathanim.GetInt() > -1 && (m_iDamageCustom == TF_DMG_CUSTOM_EMP_EXPLOSION || RandomFloat() < 0.25f))) )
	{
	...
	if ( cl_ragdoll_physics_enable.GetBool() && !bPlayDeathAnim )
	{
		// Make us a ragdoll..
		m_nRenderFX = kRenderFxRagdoll;
...
m_iv_angEyeAngles( "C_TFPlayer::m_iv_angEyeAngles" )
{
	...
	m_pSleepyEffect = NULL;
	m_pPoisonEffect = NULL;
	m_pArmorMeltEffect = NULL;
	m_pInfectionEffects[INFECTION_PART_HEAD] = NULL;
	m_pInfectionEffects[INFECTION_PART_BODY] = NULL;
...
int C_TFPlayer::ArmorValue() const
{
	return m_ArmorValue;
}
...
void C_TFPlayer::OnDataChanged( DataUpdateType_t updateType )
{
...
#ifdef PF2_CLIENT
	if( IsLocalPlayer() && (m_ArmorValue != m_iOldArmor || m_iHealth != m_iOldHealth ) )
	{
		DoPlayerStatusAlerts();
	}
#endif
	...
	{
		if (pf_burningplayerlight.GetBool())
		{
			dlight_t *dl = effects->CL_AllocDlight( LIGHT_INDEX_PLAYER_BRIGHT + index );
			dl->flags = DLIGHT_NO_MODEL_ILLUMINATION;
			dl->origin = GetLocalOrigin() + Vector( 0, 0, 48 ); //make it somewhere inside the player
			dl->color.r = 255;
			dl->color.g = 100;
			dl->color.b = 30;
			dl->color.exponent = 4;
	...
	if( m_Shared.InCond( TF_COND_TRANQUILIZED ) && !m_pSleepyEffect )
	{
		CreateSleepyEffect();
	}
	if (m_Shared.InCond( TF_COND_HALLUCINATING ) && !m_pPoisonEffect)
	{
		CreatePoisonEffect();
	}
	if( m_Shared.InCond( TF_COND_ARMOR_MELTING ) && !m_pArmorMeltEffect )
	{
		CreateArmorMeltEffect();
	}
	if( m_Shared.InCond( TF_COND_INFECTED ) && !(m_pInfectionEffects[INFECTION_PART_HEAD] || m_pInfectionEffects[INFECTION_PART_BODY]) )
	{
...
void C_TFPlayer::ThirdPersonSwitch( bool bThirdPerson )
{
	...
	if( pWeapon )
	{
		...
		if( pWeapon->m_hMuzzleFlashModel[0] )
		{
			pWeapon->m_hMuzzleFlashModel[0].Get()->SetLifetime( 0 );
			pWeapon->m_hMuzzleFlashModel[0] = NULL;
		}
	}
#endif
...
void C_TFPlayer::ClientThink()
{
	...
	if( m_pSaveMeEffect )
	{
		...
		if( !IsAlive() ||
			(IsLocalPlayer() && InFirstPersonView() && !m_Shared.InCond(TF_COND_TAUNTING)) || /*TODO - this isn't 100% */
	...
	if ( m_pSleepyEffect )
	{
		if( !IsAlive() ||
			(IsLocalPlayer() && InFirstPersonView()) ||
			(m_Shared.InCond( TF_COND_DISGUISED ) && IsEnemyPlayer() && (GetPercentInvisible() > 0)) ||
			!m_Shared.InCond( TF_COND_TRANQUILIZED ) )
		{
			ParticleProp()->StopEmission( m_pSleepyEffect );
			m_pSleepyEffect = NULL;
		}
	}
	...
	if ( m_pPoisonEffect )
	{
		if ( !IsAlive() ||
			(IsLocalPlayer() && InFirstPersonView()) ||
			(m_Shared.InCond( TF_COND_DISGUISED ) && IsEnemyPlayer() && GetPercentInvisible() > 0) ||
			!m_Shared.InCond( TF_COND_HALLUCINATING ))
		{
			ParticleProp()->StopEmission( m_pPoisonEffect );
			m_pPoisonEffect = NULL;
		}
	}

	if( m_pArmorMeltEffect )
	{
		if( !IsAlive() ||
			(IsLocalPlayer() && InFirstPersonView()) ||
			(m_Shared.InCond( TF_COND_DISGUISED ) && IsEnemyPlayer() && GetPercentInvisible() > 0) ||
			!m_Shared.InCond( TF_COND_ARMOR_MELTING ) )
		{
			ParticleProp()->StopEmission( m_pArmorMeltEffect );
			m_pArmorMeltEffect = NULL;
		}
	}

	if( m_pInfectionEffects[INFECTION_PART_HEAD] || m_pInfectionEffects[INFECTION_PART_BODY] )
	{
		if( !IsAlive() ||
			( IsLocalPlayer() && InFirstPersonView() ) ||
			( m_Shared.InCond( TF_COND_DISGUISED ) && IsEnemyPlayer() && GetPercentInvisible() > 0 ) ||
			!m_Shared.InCond( TF_COND_INFECTED ) )
		{
			if( m_pInfectionEffects[INFECTION_PART_HEAD] )
			{
				ParticleProp()->StopEmissionAndDestroyImmediately( m_pInfectionEffects[INFECTION_PART_HEAD] );
				m_pInfectionEffects[INFECTION_PART_HEAD] = NULL;
			}

			if( m_pInfectionEffects[INFECTION_PART_BODY] )
			{
				ParticleProp()->StopEmission( m_pInfectionEffects[INFECTION_PART_BODY] );
				m_pInfectionEffects[INFECTION_PART_BODY] = NULL;
			}
		}
	}
...
void C_TFPlayer::UpdateLookAt( void )
{
	...
	if( bFoundViewTarget == false  || m_nRenderFX != kRenderFxRagdoll)
	{
...
void C_TFPlayer::ClientPlayerRespawn( void )
{
	if( IsLocalPlayer() )
	{
		...
		C_TFPlayer *pPlayer = ToTFPlayer( GetLocalPlayer() );
		...
		//Update it when we spawn since the crits effect can sometimes be red or blue
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_TFPlayer::CreateSaveMeEffect( void /*bool bAutoCall*/)
{
	...
	if( pLocalPlayer && GetTeamNumber() == pLocalPlayer->GetTeamNumber() && pLocalPlayer->IsPlayerClass( TF_CLASS_ENGINEER ) )
		m_pSaveMeEffect = ParticleProp()->Create( "speech_engicall", PATTACH_POINT_FOLLOW, "head" );
	else
	...
	if( pLocalPlayer && 
		...
		|| pLocalPlayer->IsPlayerClass( TF_CLASS_ENGINEER ) ) 
		&& pLocalPlayer->IsAlive() )
	{
		...
		if( GetAttachmentLocal( LookupAttachment( "head" ), vecPos ) )
		{
			...
			// Send MedicCallType_t
			// Priority is burning > infection > low health > default
			int callType = 0;

			if( pLocalPlayer->IsPlayerClass( TF_CLASS_MEDIC ) )
			{
				if( m_Shared.InCond( TF_COND_BURNING ) )
					callType = MEDICCALL_BURNING;
				else if( m_Shared.InCond( TF_COND_INFECTED ) )
					callType = MEDICCALL_BLEEDING;
				else if( GetHealth() < GetMaxHealth() / 2.0f )
					callType = MEDICCALL_LOWHEALTH;

				CTFMedicCallerPanel::AddMedicCaller( this, 5.0, vecPos, callType );
			}
			else
			{
				if( m_Shared.InCond( TF_COND_HALLUCINATING ) )
					callType |= ENGICALL_MELTING;
				if( ArmorValue() < GetPlayerClass()->GetMaxArmor() / 2.0f )
					callType |= ENGICALL_LOWARMOR;

				CTFMedicCallerPanel::AddEngiCaller( this, 5.0, vecPos, callType );
			}
		}
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_TFPlayer::CreateSleepyEffect( void )
{
	// Don't create them for the local player
	if( IsLocalPlayer() && !ShouldDrawLocalPlayer() )
		return;
	...
	m_pSleepyEffect = ParticleProp()->Create( "tranq_sleepy", PATTACH_POINT_FOLLOW, "head" );
...
void C_TFPlayer::CreatePoisonEffect( void )
{
	...
	m_pPoisonEffect = ParticleProp()->Create( "gas_poison", PATTACH_POINT_FOLLOW, "head" );
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_TFPlayer::CreateArmorMeltEffect( void )
{
	// Don't create them for the local player
	if( IsLocalPlayer() && !ShouldDrawLocalPlayer() )
		return;
	...
	m_pArmorMeltEffect = ParticleProp()->Create( "armor_ash", PATTACH_ABSORIGIN_FOLLOW );
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_TFPlayer::CreateInfectionEffect( void )
{
	// Don't create them for the local player
	if( IsLocalPlayer() && !ShouldDrawLocalPlayer() )
		return;
	...
	if( m_pInfectionEffects[INFECTION_PART_HEAD] )
	{
		ParticleProp()->StopEmission( m_pInfectionEffects[INFECTION_PART_HEAD]);
		m_pInfectionEffects[INFECTION_PART_HEAD] = NULL;
	}

	if( m_pInfectionEffects[INFECTION_PART_BODY] )
	{
		ParticleProp()->StopEmissionAndDestroyImmediately( m_pInfectionEffects[INFECTION_PART_BODY] );
		m_pInfectionEffects[INFECTION_PART_BODY] = NULL;
	}

	m_pInfectionEffects[INFECTION_PART_HEAD] = ParticleProp()->Create("medic_infection", PATTACH_POINT_FOLLOW, "head");
	m_pInfectionEffects[INFECTION_PART_BODY] = ParticleProp()->Create("infection_floating_specs", PATTACH_ABSORIGIN_FOLLOW);

}

void C_TFPlayer::DoPlayerStatusAlerts( void )
{
	if( !IsAlive() || m_iHealth <= 0 )
		return;

	bool bArmorAlerts = pf_alerts_armor.GetBool();

	bool bLowHealth = (m_iHealth / (float)GetMaxHealth()) <= (float)(pf_alerts_health_percentage.GetFloat() / 100.0f);
	bool bDepleted = m_ArmorValue <= 0;
	float flPerc = m_ArmorValue / (float)(GetPlayerClass()->GetMaxArmor());
	float flPercLow = pf_alerts_armor_percentage.GetFloat() / 100.0f;

	bool bPlayArmorAlert = false;
	// Should we attempt an alert?
	if( bDepleted && bArmorAlerts && !m_bPlayedDepletedArmorAlert )
	{
		m_bPlayedLowArmorAlert = true;
		m_bPlayedDepletedArmorAlert = true;
		bPlayArmorAlert = true;
	}
	else if( flPerc <= flPercLow && bArmorAlerts && !m_bPlayedLowArmorAlert )
	{
		m_bPlayedLowArmorAlert = true;
		bPlayArmorAlert = true;
	}
	else
	{
		// don't play an alert
		if( flPerc > 0.0f )
			m_bPlayedDepletedArmorAlert = false;
		if( flPerc > flPercLow )
			m_bPlayedLowArmorAlert = false;
	}

	bool bPlayHealthAlert = false;
	if( bLowHealth && pf_alerts_health.GetBool() && !m_bPlayedLowHealthAlert )
	{
		m_bPlayedLowHealthAlert = true;
		bPlayHealthAlert = true;
	}
	else
	{
		if( !bLowHealth )
			m_bPlayedLowHealthAlert = false;
	}
	...
	if( bPlayArmorAlert )
	{
		EmitSound_t params;
		params.m_nPitch = 100;
		params.m_flVolume = pf_alerts_armor_volume.GetFloat();

		if( bDepleted )
			params.m_pSoundName = "ui/alerts/alert_armor_depleted.wav";
		else
			params.m_pSoundName = "ui/alerts/alert_armor_low.wav";
	...
	if( bPlayHealthAlert )
	{
		EmitSound_t params;
		params.m_nPitch = 100;
		params.m_flVolume = pf_alerts_health_volume.GetFloat();
		params.m_pSoundName = "ui/alerts/low_health_alert.wav";
...
bool C_TFPlayer::ShouldShowNemesisIcon()
{
	...
	{
		bool bStealthed = m_Shared.InCond( TF_COND_STEALTHED ) || m_Shared.InCond( TF_COND_SMOKE_BOMB );
...
void C_TFPlayer::UpdateTypingBubble( void )
{
	...
	if( !pf_showchatbubbles.GetBool() || !GetTFChatHud() )
		return;

	bool bTeamSay = !( GetTFChatHud()->GetMessageMode() == MM_SAY );

	// for all chat, show it over anyone's head.
	if (!InFirstPersonView() && m_bTyping && IsAlive() && ( bTeamSay ? !IsEnemyPlayer() : !(m_Shared.IsStealthed() && IsEnemyPlayer() ) ) )
	{
		if( !m_pTypingEffect )
			m_pTypingEffect = ParticleProp()->Create( "speech_typing", PATTACH_POINT_FOLLOW, "head" );
	}
	else if ( m_pTypingEffect )
	{
		ParticleProp()->StopEmissionAndDestroyImmediately( m_pTypingEffect );
		m_pTypingEffect = NULL;
	}
...
/*CON_COMMAND(getweapons, "Get a list of the current weapons")
{
...
Msg(a);
Msg(": ");
CBaseCombatWeapon *pWeapon = C_TFPlayer::GetLocalTFPlayer()->GetWeapon(i);
if (pWeapon)
{
...
void C_TFPlayer::DoHallucination()
{
	SetContextThink(&C_TFPlayer::DoHallucination, gpGlobals->curtime + RandomFloat(0.0f, 0.5f), "HallucinationThink");
	if (!m_Shared.IsHallucinating() || !IsLocalPlayer() || !IsAlive())
		return;

	float explosion_time = gpGlobals->curtime + 0.5f;

	int rand = RandomInt( TF_HALLUCINATION_EXPLOSION, TF_HALLUCINATION_GRENADE);
	model_t *model = NULL;
	C_LocalTempEntity *pGrenade;
	trace_t tr;
	Vector forward;
	Vector Random = RandomVector( -90.0f, 50.0f );
	Vector endtrace;
	switch( rand )
	{
	case TF_HALLUCINATION_EXPLOSION:
		EmitSound( "BaseExplosionEffect.Sound" );
		break;
	case TF_HALLUCINATION_ONFIRE:
		if( GetWaterLevel() < WL_Waist )
		{
			...
			EmitSound( "TFPlayer.FirePain" );
		}
		break;
	case TF_HALLUCINATION_EXTINGUISHFIRE:
		view->SetScreenOverlayMaterial( NULL );
		break;
	case TF_HALLUCINATION_SHOTGUNSHOT:
		EmitSound( "Weapon_Shotgun.Single" );
		break;
	case TF_HALLUCINATION_MELEEHIT_WORLD:
		EmitSound( "Weapon_Bat.HitWorld" );
		break;
	case TF_HALLUCINATION_MELEEHIT_FLESH:
		EmitSound( "Weapon_Bat.HitFlesh" );
		break;
	case TF_HALLUCINATION_DEATHSCREAM:
		EmitSound( "TFPlayer.CritDeath" );
		break;
	case TF_HALLUCINATION_GRENADE:
		EyeVectors( &forward );
		endtrace = Weapon_ShootPosition() + (forward * 300) + RandomVector( -90.0f, 50.0f );
		UTIL_TraceLine( Weapon_ShootPosition(), endtrace, MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);
		//NDebugOverlay::Line( Weapon_ShootPosition(), endtrace, 255, 255, 0, false, 10.0f );
		switch( RandomInt( 0, 2 ) )
		{
		case 0:
			model = (model_t *)engine->LoadModel( "models/weapons/w_models/w_grenade_frag.mdl" );
			break;
		case 1:
			model = (model_t *)engine->LoadModel( "models/weapons/w_models/w_grenade_conc.mdl" );
			break;
		case 2:
			model = (model_t *)engine->LoadModel( "models/weapons/w_models/w_grenade_nail.mdl" );
			break;
		}
		pGrenade = tempents->SpawnTempModel( model, Vector( tr.endpos.x, tr.endpos.y, tr.endpos.z + 70.0f ), GetAbsAngles(), Vector( -GetAbsVelocity().x, -GetAbsVelocity().y, GetAbsVelocity().z + 30.0f ), 5.0f, FTENT_COLLIDEALL );
		if( !pGrenade )
			break;
		pGrenade->SetGravity( 0.4f );
		pGrenade->flags = FTENT_COLLIDEWORLD | FTENT_GRAVITY;
		pGrenade->SetCollisionGroup( COLLISION_GROUP_INTERACTIVE );
		pGrenade->m_nSkin = (GetTeamNumber() == TF_TEAM_RED) ? 1 : 0;
		pGrenade->EmitSound( "Weapon_Grenade.FinalBeep" );
		if( pGrenade && RandomInt(0,1) == 1 && gpGlobals->curtime + 0.5f >= explosion_time )
		{
			pGrenade->AddParticleEffect( "ExplosionCore_MidAir" );
			pGrenade->ParticleProp()->Create( pGrenade->m_pszParticleEffect, PATTACH_ABSORIGIN_FOLLOW );
			pGrenade->EmitSound( "BaseExplosionEffect.Sound" );
			pGrenade->SetEFlags( EF_BRIGHTLIGHT );
			pGrenade->CreateLightEffects();
			pGrenade->SetRenderMode( kRenderNone );
		}
		//pGrenade->SetParticleEffect( "achieved" );
		
		pGrenade->PhysicsSimulate();
		break;
	default:
		model = NULL;
		pGrenade = NULL;
...
{
	C_BaseEntity::EmitSound( "Achievement.Earned" );
}