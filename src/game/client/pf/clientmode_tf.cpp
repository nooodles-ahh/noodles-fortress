...
void CTFModeManager::LevelInit( const char *newmap )
{
	...
	engine->AddPhonemeFile( "scripts/game_sounds_vo_phonemes_mod.txt" );
	engine->AddPhonemeFile( "scripts/game_sounds_vo_phonemes_local.txt" );
	engine->AddPhonemeFile( 0 );
}
...
//#endif
	...
	if ( gameUIFactory )
	{
		...
		if ( NULL != m_pGameUI )
		{
			CPFLoadingPanel *pPanel = GPFLoadingPanel();
