...
void TF_3rdPersonMuzzleFlashCallback_SentryGun( const CEffectData &data )
{
	...
	if ( pEnt && !pEnt->IsDormant() )
	{
		...
		if ( pf_muzzlelight.GetBool() )
		{
			...
			TE_DynamicLight( filter, 0.0f, &data.m_vOrigin, 255, 192, 64, 5, 70.0f, 0.05f, 70.0f / 0.05f, LIGHT_INDEX_MUZZLEFLASH );
		}
	}
}
...
//-----------------------------------------------------------------------------
C_MuzzleFlashModel *C_MuzzleFlashModel::CreateMuzzleFlashModel( const char *pszModelName, C_BaseEntity *pParent, int iAttachment, float flLifetime, float flScale, bool bVMRenderGroup )
{
	C_MuzzleFlashModel *pFlash = new C_MuzzleFlashModel;
	if ( !pFlash )
		return NULL;

	if ( !pFlash->InitializeMuzzleFlash( pszModelName, pParent, iAttachment, flLifetime, flScale, bVMRenderGroup ) )
		return NULL;
...
// Purpose: 
//-----------------------------------------------------------------------------
bool C_MuzzleFlashModel::InitializeMuzzleFlash( const char *pszModelName, C_BaseEntity *pParent, int iAttachment, float flLifetime, float flScale, bool bVMRenderGroup )
{
	m_bIs3rdPersonFlash = true;

	// Probably a better way to decide all this, but it works
	C_BaseViewModel *pVM = dynamic_cast<C_BaseViewModel *>(pParent);
	if(pVM)
		m_bIs3rdPersonFlash = false;

	m_bVMRenderGroup = bVMRenderGroup;
		
	AddEffects( EF_NORECEIVESHADOW | EF_NOSHADOW );
	if ( InitializeAsClientEntity( pszModelName, RENDER_GROUP_VIEW_MODEL_TRANSLUCENT ) == false )
	{
		Release();
		return false;
	}
	...
	m_flRotateAt = gpGlobals->curtime + flLifetime;
...
{
	if ( !GetMoveParent() || (m_flExpires && gpGlobals->curtime > m_flExpiresAt) )
	{
		Release();
		return;
	}
...
//-----------------------------------------------------------------------------
RenderGroup_t C_MuzzleFlashModel::GetRenderGroup()
{
	if( !m_bIs3rdPersonFlash && m_bVMRenderGroup )
