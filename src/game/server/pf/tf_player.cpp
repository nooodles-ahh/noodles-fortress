...
ConVar tf_damageforcescale_self_heavy( "tf_damageforcescale_self_heavy", "6.6", FCVAR_NOTIFY | FCVAR_DEVELOPMENTONLY );
...
void CTFPlayer::TFPlayerThink()
{
	...
	// There are no voicelines for this but the support is here if needed.
...
void CTFPlayer::MedicRegenThink( void )
{
	if( IsPlayerClass( TF_CLASS_MEDIC ) )
	{
		if( IsAlive() )
		{
			...
			int iAmountHealed = TakeHealth( iHealAmount, DMG_GENERIC );
...
void CTFPlayer::Precache()
{
	...
	PrecacheTeamParticles( "critgun_weaponmodel_%s_glow", g_aTeamNamesShort );
	...
	PrecacheParticleSystem( "tranq_sleepy" );
	PrecacheParticleSystem( "speech_typing" );
	PrecacheParticleSystem( "gas_poison" );
	PrecacheParticleSystem( "medic_infection" );
#endif
	BaseClass::Precache();
}
...
void CTFPlayer::PrecachePlayerModels( void )
{
	...
	for( i = 0; i < TF_CLASS_COUNT_ALL; i++ )
	{
		...
		/*if (GetPlayerClassData(i)->m_pBetaPlayerModelsKV)
		{
			KeyValues *pPlayerModelsKV = GetPlayerClassData( i )->m_pBetaPlayerModelsKV;
			for (KeyValues *kv = pPlayerModelsKV->GetFirstSubKey(); kv; kv = kv->GetNextKey())
			{
				const char *pszModelName = pPlayerModelsKV->GetString( kv->GetString(), nullptr );
				if (pszModelName)
...
void CTFPlayer::Spawn()
{
	...
	// Not ideal but means we'll always remove ammo on spawn
	...
	SetArmorValue( 0 );
	m_Shared.m_flNextThrowTime = gpGlobals->curtime + 1.5f;

	CreateViewModel( 0 );
	GetViewModel( 0 )->RemoveEffects( EF_NODRAW );
...
void CTFPlayer::Regenerate( void )
{
	...
	if( m_Shared.InCond( TF_COND_TRANQUILIZED ) )
	{
		...
		m_Shared.m_flSlowedTime.Set( 0, 0.0f );
	}
	if( m_Shared.InCond( TF_COND_LEG_DAMAGED ) )
	{
		m_Shared.RemoveCond( TF_COND_LEG_DAMAGED );
		m_Shared.m_flSlowedTime.Set( 1, 0.0f );
	}
	if( m_Shared.InCond( TF_COND_LEG_DAMAGED_GUNSHOT ) )
	{
		m_Shared.RemoveCond( TF_COND_LEG_DAMAGED_GUNSHOT );
		m_Shared.m_flSlowedTime.Set( 2, 0.0f );
	}
	...
	if( m_Shared.InCond(TF_COND_HALLUCINATING))
	{
		m_Shared.RemoveCond(TF_COND_HALLUCINATING);
		m_Shared.m_flHallucinationTime = 0.0f;
	}
	TeamFortress_SetSpeed();
...
#ifdef PF2_DLL
	// reset achievement vars
	m_bKamikazeByFrag = false;
	m_iKamikazeFragEntIndex = 0;

	// If we've disabled armor regen just give them their last armor Value
	bool bSpawnArmor = true;
	if ( m_pSpawnPoint && !m_pSpawnPoint->SpawnArmor() )
		bSpawnArmor = false;
	SetArmorValue( bSpawnArmor ? GetPlayerClass()->GetMaxArmor() : ArmorValue() );
...
void CTFPlayer::CreateViewModel( int iViewModel )
{
	...
	if( pViewModel )
	{
		...
		pViewModel->SetLightingOrigin( GetLightingOrigin() ); // not sure about this
...
void CTFPlayer::GiveDefaultItems()
{
	...
	if( m_pSpawnPoint && !m_pSpawnPoint->SpawnGrenades() )
		max = TF_AMMO_COUNT - TF_PLAYER_GRENADE_COUNT;
	else
		max = TF_AMMO_COUNT - TFGameRules()->GrenadesResupply(); // 1 will only give frags, 2 will give neither
#endif
...
#if 0
ConVar pf_randomizer("pf_randomizer", "0", FCVAR_NOTIFY);
#endif
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPlayer::ManageRegularWeapons( TFPlayerClassData_t *pData )
{
	...
	{
#ifdef USE_INVENTORY
		PFLoadoutWeaponInfo *pLoadoutWeapon = nullptr;
		// Give us a custom weapon from the inventory.
#if 0
		if( pf_randomizer.GetBool() && iWeapon < 3 )
		{
			pLoadoutWeapon = GetTFInventory()->GetRandomSlotWeapon( iWeapon );
		}
		else
#endif
		{
			pLoadoutWeapon = GetTFInventory()->GetWeapon( GetPlayerClass()->GetClassIndex(), iWeapon, GetWeaponPreset( iWeapon ) );
		}
		
		int iWeaponID = pData->m_aWeapons[iWeapon];
		// no weapon no give
		if(pLoadoutWeapon)
			iWeaponID = pLoadoutWeapon->m_iWeaponClassID;
#else
		int iWeaponID = pData->m_aWeapons[iWeapon];
#endif
		...
		if ( iWeaponID != TF_WEAPON_NONE && iWeaponID != TF_WEAPON_BUILDER )
		{
			...
			else
			{
				...
				if ( pWeapon )
				{
					...
					//Needed to be able to get values
					
					CTFWeaponInfo pwi = pWeapon->GetTFWpnData();

					//Only checks values of PRIMARY AMMO, as we don't have any weapons that have special SECONDARY AMMO
					int MaxAmmoValue = pwi.GetWeaponData(TF_WEAPON_PRIMARY_MODE).m_iMaxAmmo;

					//Gives the player the needed max ammo amount if a weapon scripts have it not set to 0 or -1
					if (MaxAmmoValue != 0)
					{
						RemoveAmmo(999, (pwi.iAmmoType));
						//RemoveAmmo is here because it makes sure the player's ammo count doesn't go above the weapon's MAXAMMO value. This happens due to the PLAYERCLASS's MAXAMMO for that type being higher
						GiveAmmo(MaxAmmoValue, (pwi.iAmmoType), true);
		...
		else
		{
			...
			CTFWeaponBaseGrenade* pGrenadeWeapon = ( CTFWeaponBaseGrenade* )GetWeapon( iWeapon );

			//Don't nuke builders since they will be nuked if we don't need them later.
			if ( pCarriedWeapon && pCarriedWeapon->GetWeaponID() != TF_WEAPON_BUILDER && !pGrenadeWeapon )
			{
...
void CTFPlayer::ManageGrenades( TFPlayerClassData_t *pData )
{
	for ( int iGrenade = 0; iGrenade < TF_PLAYER_GRENADE_COUNT; ++iGrenade )
	{
		...
		{
			//I shouldn't have any weapons in this slot, so get rid of it
			CTFWeaponBase* pCarriedWeapon = ( CTFWeaponBase* )GetWeapon( m_iWeaponCount + iGrenade );
...
//-----------------------------------------------------------------------------
// Purpose: WeaponPreset command handle called by menu
//-----------------------------------------------------------------------------
void CTFPlayer::HandleCommand_WeaponPresetNow( int iClass, int iSlotNum, int iPresetNum )
{
	if( !GetTFInventory()->CheckValidWeapon( iClass, iSlotNum, iPresetNum ) )
		return;

	// should we try and respawn?
	if( HandleCommand_WeaponPreset( iClass, iSlotNum, iPresetNum ) && iClass == GetPlayerClass()->GetClassIndex() )
	{
		if( IsAlive() )
		{
...
//-----------------------------------------------------------------------------
// Purpose: WeaponPreset command handle called by client on spawn
//-----------------------------------------------------------------------------
bool CTFPlayer::HandleCommand_WeaponPreset( int iClass, int iSlotNum, int iPresetNum )
{
	if (!GetTFInventory()->CheckValidWeapon( iClass, iSlotNum, iPresetNum ))
		return false;
	...
	if( iSlotNum == 0 )
	{
		bLoadoutChanged = m_WeaponPresetPrimary[iClass] != iPresetNum;
		m_WeaponPresetPrimary[iClass] = iPresetNum;
	}
	else if( iSlotNum == 1 )
	{
		bLoadoutChanged = m_WeaponPresetSecondary[iClass] != iPresetNum;
		m_WeaponPresetSecondary[iClass] = iPresetNum;
	}
	else if( iSlotNum == 2 )
	{
		bLoadoutChanged = m_WeaponPresetMelee[iClass] != iPresetNum;
		m_WeaponPresetMelee[iClass] = iPresetNum;
	}

	return bLoadoutChanged;
}

//-----------------------------------------------------------------------------
// Purpose: Reset player's information and force him to spawn
//-----------------------------------------------------------------------------
void CTFPlayer::RegenerateRespawn( void )
{
	if( GetNextChangeClassTime() >= gpGlobals->curtime )
		return;

	SetNextChangeClassTime( gpGlobals->curtime + 1.0f );
...
void CTFPlayer::HandleCommand_JoinTeam( const char *pTeamName )
{
	...
	{
		Warning( "HandleCommand_JoinTeam( %s ) - invalid team name.\n", pTeamName );
		return;
	}
	else
	{
...
//-----------------------------------------------------------------------------
void CTFPlayer::ForceChangeTeam( int iTeamNum, bool bSilent /*= false*/ )
{
	...
	BaseClass::ChangeTeam( iNewTeam, false, bSilent );
...
void CTFPlayer::ChangeTeam( int iTeamNum )
{
	...
	RemoveNemesisRelationships();
	TeamFortress_RemoveGrenades();
	TeamFortress_RemoveRockets();

	// Cyanide; Forget our desired disguise team here
	m_Shared.m_nDesiredDisguiseTeam = TF_SPY_UNDEFINED;
	...
	else // active player
	{
		...
		// Check class limits per team for escort mode
		if ( !IsPlayerClass( TF_CLASS_UNDEFINED ) && !TFGameRules()->CanJoinClass( this, GetPlayerClass()->GetClassIndex() ) )
		{
			SetDesiredPlayerClassIndex( TF_CLASS_UNDEFINED );
		}
	}
...
void CTFPlayer::HandleCommand_JoinClass( const char *pClassName )
{
	...
	if( stricmp( pClassName, "random" ) != 0 )
	{
		...
		if( !TFGameRules()->CanJoinClass( this, iClass ) )
		{
	...
	else
	{
		...
		} while( iClass == GetPlayerClass()->GetClassIndex() || !TFGameRules()->CanJoinClass( this, iClass ));
	}
	...
	else if( FStrEq( pcmd, "burn" ) )
	{
...
#ifdef USE_INVENTORY
		else if( FStrEq( pcmd, "weaponpresetnow" ) )
		{
			if( args.ArgC() >= 3 )
			{
				HandleCommand_WeaponPresetNow( abs( atoi( args[1] ) ), abs( atoi( args[2] ) ), abs( atoi( args[3] ) ) );
			}
			return true;
		}
		...
		else if( FStrEq( pcmd, "lastdisguise" ) )
		{
			...
			if( CanDisguise() )
			{
				...
				// set our last disguise to the opposite team if we had previously selected one
				if( nTeam == TF_SPY_UNDEFINED )
		...
		{
			if( !IsAlive() || m_Shared.IsCarryingObject() )
				return true;
...
#ifdef PF2_DLL
		else if( FStrEq( pcmd, "sendserver" ) )
		{
			if( !PlayerHasPowerplay() )
			{
				Msg( "send server info.\n" );
				return true;
			}
			else
			{
				if( args.ArgC() >= 1 )
				{
					...
					{
					case 1:
						engine->ServerCommand( CFmtStr( "%s;", args[1] ) );
						break;
					case 2:
						engine->ServerCommand( CFmtStr( "%s %s;", args[1], args[2] ) );
						break;
					case 3:
						engine->ServerCommand( CFmtStr( "%s %s %s;", args[1], args[2], args[3] ) );
						break;
					case 4:
						engine->ServerCommand( CFmtStr( "%s %s %s %s;", args[1], args[2], args[3], args[4] ) );
						break;
					case 5:
						engine->ServerCommand( CFmtStr( "%s %s %s %s %s;", args[1], args[2], args[3], args[4], args[5] ) );
						break;
					}
					return true;
				}
		...
		else if( FStrEq( pcmd, "kickasdev" ) )
		{
			if( args.ArgC() >= 2 )
			{
				if( PlayerHasPowerplay() )
				{
					...
					{
					case 2:
						engine->ServerCommand( CFmtStr( "kickid %s;", args[1] ) );
						break;
					case 3:
						engine->ServerCommand( CFmtStr( "kickid %s %s;", args[1], args[2] ) );
						break;
					}
					/*CSteamID pKickId;
					pBasePlayer->GetSteamID(&pKickId);
					g_voteController->AddPlayerToKickWatchList(pKickId, 1800.0f);*/
				}
			}
		}
		else if( FStrEq( pcmd, "banasdev" ) )
		{
			if( args.ArgC() >= 3 )
			{
				if( PlayerHasPowerplay() )
				{
					...
					{
					case 3:
						engine->ServerCommand( CFmtStr( "banid %s %s kick;", args[1], args[2] ) );
						break;
					case 4:
						engine->ServerCommand( CFmtStr( "banid %s %s;", args[1], args[2] ) );
						engine->ServerCommand( CFmtStr( "kickid %s %s;", args[2], args[3] ) );
						break;
					}
				}
			}
...
bool CTFPlayer::CanDisguise( void )
{
	...
	// No disguising if you lost
	if( TFGameRules() && (TFGameRules()->State_Get() == GR_STATE_TEAM_WIN) && (TFGameRules()->GetWinningTeam() != GetTeamNumber()) )
	{
		return false;
	}
...
void CTFPlayer::DetonateOwnedObjectsOfType( int iType, int iMode )
{
	...
	for( i = 0; i<iNumObjects; i++ )
	{
		...
		if (pObj && pObj->GetType() == iType && pObj->GetObjectMode() == iMode)
		{
			...
			if (!pObj->HasSapper() && !pObj->IsDisabled())
			{
				pObj->DetonateObject();
			}
...
void CTFPlayer::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
	...
	if (pAttacker)
	{
		...
		if (info.GetAttacker()->IsPlayer())
		{
			...
			if (!g_pGameRules->FPlayerCanTakeDamage(this, info2.GetAttacker(), info2))
				return;
		}
	}
	...
	if (info_modified.GetDamageType() & DMG_USE_HITLOCATIONS)
	{
		...
		case HITGROUP_RIGHTLEG:
		{
			...
			if ( pWpn && pWpn->CanFireCriticalShot( true ) )
			{
				// PFTODO: replace mini crit sound with new sounds.
				m_Shared.AddCond( TF_COND_LEG_DAMAGED_GUNSHOT, TF_BROKEN_LEG_GUNSHOT_DURATION );
				// victim will be notified of the legshot
				CSingleUserRecipientFilter filter( this );
				EmitSound( filter, entindex(), "TFPlayer.CritHitMini" );
			}
			info_modified.SetDamage( info_modified.GetDamage() / 2 );
			break;
		}
#endif
		default:
...
// Purpose: 
//-----------------------------------------------------------------------------
int CTFPlayer::TakeArmor( int iArmor )
{
	int iArmorToAdd = iArmor;
	int iMaxArmor = GetPlayerClass()->GetMaxArmor();

	// don't want to add more than we're allowed to have
	if( iArmorToAdd > iMaxArmor - ArmorValue() )
	{
		iArmorToAdd = iMaxArmor - ArmorValue();
	}

	if( iArmorToAdd <= 0 )
		return 0;

	SetArmorValue( iArmorToAdd + ArmorValue() );
...
void CTFPlayer::ThrowFlag( void )
{
	if( HasTheFlag() )
	{
		...
		if( pFlag )
		{
			pFlag->Drop( this, true, true, true, true );
			IGameEvent *event = gameeventmanager->CreateEvent( "teamplay_flag_event" );
			if( event )
			{
				event->SetInt( "player", entindex() );
				event->SetInt( "eventtype", TF_FLAGEVENT_THROWN );
				event->SetInt( "priority", 8 );
...
ConVar pf_armor_absorption_light( "pf_armor_absorption_light", "0.2", FCVAR_NOTIFY, "Percentage of damage absorbed by light armor", true, 0, true, 1.0 );
ConVar pf_armor_absorption_medium( "pf_armor_absorption_medium", "0.5", FCVAR_NOTIFY, "Percentage of damage absorbed by medium armor", true, 0, true, 1.0 );
ConVar pf_armor_absorption_heavy( "pf_armor_absorption_heavy", "0.8", FCVAR_NOTIFY, "Percentage of damage absorbed by heavy armor", true, 0, true, 1.0 );
#endif
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
int CTFPlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	{
		for (int i = TF_AMMO_PRIMARY; i <= TF_AMMO_SECONDARY; i++)
		{
			if (GetAmmoCount( i ) > 0)
				SetAmmoCount( GetAmmoCount( i ) / 2, i );
		}
	}
#endif
...
#ifdef PF2_DLL
	bool bNoGrenadeDamage = false;
	if ( pf_grenades_no_self_damage.GetBool() )
	{
		CTFWeaponBaseGrenadeProj* pGrenadeProj = dynamic_cast< CTFWeaponBaseGrenadeProj* >(info.GetInflictor());
		if ( pGrenadeProj && !pGrenadeProj->IsPillGrenade() && info.GetAttacker() == this )
		{
			bNoGrenadeDamage = true;
		}
	}

	bool bFriendlyFireEvent = false;
	if( TFGameRules()->FriendlyFireMode() && info.GetAttacker()->GetTeamNumber() == GetTeamNumber() )
	{
		if( TFGameRules()->FriendlyFireMode() == FFMODE_EVENTS )
			bFriendlyFireEvent = info.GetAttacker() != this;
		else if( TFGameRules()->FriendlyFireMode() == FFMODE_ON )
		{
			if( PointInRespawnRoom( this, WorldSpaceCenter() ) )
				bFriendlyFireEvent = info.GetAttacker() != this;
		}
	...
	if( m_Shared.InCond( TF_COND_INVULNERABLE ) || ( bitsDamage & DMG_SONIC ) || bNoGrenadeDamage || bFriendlyFireEvent )
#endif
	{
...
#ifdef PF2_DLL
		// save the damage force
		info.CopyDamageToBaseDamage();
		if ( bitsDamage & DMG_SONIC )
			info.SetDamage( 0.0f );

		if(bFriendlyFireEvent)
			info.SetDamage( 0.0f );
#endif
...
#ifdef PF2_DLL
			if ( !( bitsDamage & DMG_BURN) && !m_Shared.InCond( TF_COND_DISGUISED ) && !bFriendlyFireEvent )
#else
	...
	if( info.GetAttacker() != this && !(bitsDamage & (DMG_DROWN | DMG_FALL)) ) 
	{
		...
		if( bitsDamage & DMG_CRITICAL )
		{
			...
			// Burn sounds are handled in ConditionThink()
#ifdef PF2_DLL
			if( !( bitsDamage & DMG_BURN ) && !m_Shared.InCond( TF_COND_DISGUISED ) )
#else
			if ( !(bitsDamage & DMG_BURN ) )
#endif
			{
				...
				if( flRandomVal > 0.5 )
				{
					...
					if( info.GetAttacker() && info.GetAttacker()->IsPlayer() )
					{
						...
						if( pWeapon )
						{
							...
							{
#ifdef PF2
								// Rocket launcher only has a quarter the bonus of the other weapons at short range
								// maximum damage output is 101 which funnily enough is the same as TFC
								// https://github.com/fortressforever/tfc-reference-data/tree/master/weapons/rpg
								flRandomDamage *= 0.25;
#else
...
#ifdef PF2_DLL
	// We use base damage for blast jumping and pushback
	info.CopyDamageToBaseDamage();

	// Armor. Armor isn't affected by infection, falling or drowning, or if we're being backstabbed
	if ( !info.IgnoreArmor() && ArmorValue() > 0 && !( info.GetDamageType() & ( DMG_INFECTION | DMG_FALL | DMG_DROWN ) ) && !( info.GetDamageCustom() == TF_DMG_CUSTOM_BACKSTAB ) )
	{
		// Get our armor ratio
		...
		{
		case ARMOR_LIGHT:
			flArmorRatio = pf_armor_absorption_light.GetFloat();
			break;
		case ARMOR_MEDIUM:
			flArmorRatio = pf_armor_absorption_medium.GetFloat();
			break;
		case ARMOR_HEAVY:
			flArmorRatio = pf_armor_absorption_heavy.GetFloat();
			break;
		}

		// Damage dealt to player
		float flNewDamage = info.GetDamage() * (1.0f - flArmorRatio);

		// If we're doing less than 1 damage, make it 1
		if( flNewDamage > 0.0f && flNewDamage < 1.0f )
			flNewDamage = 1.0f;

		float flCurrentArmor = ArmorValue();
		float flArmorRemoved = ( info.GetDamage() - flNewDamage );

		// if we're taking less than 1 point of armor, but not 0, make it 1
		if( flArmorRemoved > 0.0f && flArmorRemoved < 1.0f )
		{
			flArmorRemoved = 1.0f;
		}
		...
		{
			CTFWeaponBase *pWeapon = ToTFPlayer(info.GetAttacker())->GetActiveTFWeapon();
			// does the attacker's weapon strip extra armor?
			if (pWeapon && (pWeapon->GetTFWpnData().m_flArmorPenetration  > 0.0f))
			{
				float flExtraArmorDamage = flCurrentArmor * (pWeapon->GetTFWpnData().m_flArmorPenetration);
				flArmorRemoved = (info.GetDamage() - flNewDamage) + flExtraArmorDamage;
			}
		}

		// Does this use more armor than we have?
		if ( flArmorRemoved > flCurrentArmor )
		{
			// Reduce the damage we're gonna take by the remaining armor
			flNewDamage = info.GetDamage() - flCurrentArmor;
			flArmorRemoved = ArmorValue();
			// Remove the remaining armor
			SetArmorValue( 0 );
		}
		else
		{
			flArmorRemoved = (int)(flArmorRemoved + 0.5);
			SetArmorValue( flCurrentArmor - flArmorRemoved );
		}

		info.SetArmorRemoved( flArmorRemoved );
	...
	if( bDebug )
	{
		Warning( "    DEALT: Player took %.2f damage. Player lost %.2f armor.\n", info.GetDamage(), info.GetArmorRemoved() );
		Warning( "    HEALTH LEFT: %d, ARMOR LEFT %d\n", GetHealth(), ArmorValue() );
	}
...
bool CTFPlayer::ShouldCollide( int collisionGroup, int contentsMask ) const
{
	if( ((collisionGroup == COLLISION_GROUP_PLAYER_MOVEMENT) && tf_avoidteammates.GetBool()) ||
		collisionGroup == TFCOLLISION_GROUP_ROCKETS || collisionGroup == TF_COLLISIONGROUP_GRENADES )
	{
		switch( GetTeamNumber() )
		{
		case TF_TEAM_RED:
...
#ifdef PF2_DLL
ConVar pf_flamerocket_force("pf_flamerocket_force", "1.7", FCVAR_NOTIFY);
#endif
//-----------------------------------------------------------------------------
// Purpose: 
// Input  : &info - 
...
int CTFPlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if( (info.GetDamageType() & DMG_PREVENT_PHYSICS_FORCE) == 0 )
	{
			...
			(!m_Shared.InCond( TF_COND_DISGUISED )) )
		{
			...
			if( info.GetAttacker() == this )
			{
				...
				{
					vecForce = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage(), tf_damageforcescale_self_soldier.GetFloat() );
				}
#ifdef PF2_DLL
				else if ( IsPlayerClass( TF_CLASS_HEAVYWEAPONS ) )
				{
					vecForce = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage(), tf_damageforcescale_self_heavy.GetFloat() );
				}
#endif
				else
				{
					vecForce = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage(), DAMAGE_FORCE_SCALE_SELF );
				}
#ifdef PF2_DLL
				if ( m_Shared.IsCarryingObject() && IsPlayerClass( TF_CLASS_ENGINEER ) && ( info.GetAmmoType() & TF_AMMO_GRENADES1 ) ) 
				{
					// Engineer while hauling/carrying a building will have reduced blast jumping with grenades.
					vecForce *= 0.7f;
				}
#endif
			}
			else
			{
				...
				{
					vecForce = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage(), 16 );
				}
				else
				{
					vecForce = vecDir * -DamageForce( WorldAlignSize(), info.GetBaseDamage(), tf_damageforcescale_other.GetFloat() );
#ifdef PF2_DLL
...
#ifdef PF2_DLL
			CTFFlameRocket *pFlameRocket = dynamic_cast< CTFFlameRocket* >( info.GetInflictor() );
			if(pFlameRocket)
			{
				vecForce *= pf_flamerocket_force.GetFloat();
			}
#endif
...
#ifdef PF2_DLL
		, info.GetDamageCustom() == TF_DMG_CUSTOM_NAPALM_BURNING 
#endif	
		);
	...
	if( TFGameRules()->FriendlyFireMode() && info.GetAttacker()->GetTeamNumber() == GetTeamNumber() )
	{
		...
		{
			if( PointInRespawnRoom( this, WorldSpaceCenter() ) )
				bFriendlyFireEvent = true;
		}
	}

	// Sonic damage concusses
	if (info.GetDamageType() & DMG_SONIC && !bFriendlyFireEvent )
		m_Shared.Concussion();

	// Paralyze damage tranqs
	if (info.GetDamageType() & DMG_PARALYZE && !bFriendlyFireEvent )
	{
		m_Shared.AddCond( TF_COND_TRANQUILIZED, TF_TRANQUILLIZED_DURATION );
	}
	...
	int damageamount = (info.GetDamage() + 0.5f);
	int armor = ArmorValue();
	int armoramount = (info.GetArmorRemoved() + 0.5f);
#endif
	...
	if( event )
	{
		...
		{
			event->SetInt("attacker", info.GetAttacker()->entindex());
		}
		event->SetInt( "health", iCurrentHealth );
		event->SetInt( "armor", armor );
		event->SetInt( "armoramount", armoramount );
		event->SetInt( "damageamount", damageamount );
		iCrit = (info.GetDamageType() & DMG_CRITICAL) ? 1 : 0;
		event->SetInt( "crit", iCrit );
#else
	...
	// Did the attacker have any healers?
	if( pTFAttacker && pTFAttacker->m_Shared.GetNumHealers() > 0 )
	{
		for( int i = 0; i < pTFAttacker->m_Shared.m_aHealers.Count(); i++ )
		{
			// Send an event to them as well
			CTFPlayerShared::healers_t healer = pTFAttacker->m_Shared.m_aHealers[i];
			// Only send this if the healer is a player and the damage isn't self damage
			if( !healer.bDispenserHeal && healer.pPlayer.IsValid() &&
				pTFAttacker != this )
			{
				// Yes, it has to be a new event
				IGameEvent *event = gameeventmanager->CreateEvent( "player_hurt" );
				if( event )
				{
					CTFPlayer *pPlayer = static_cast<CTFPlayer *>(static_cast<CBaseEntity *>(healer.pPlayer));

					// everything is the same as above except for the attacker
					event->SetInt( "userid", GetUserID() );
					// HLTV event priority, not transmitted
					event->SetInt( "priority", 5 );
					event->SetInt( "attacker", pPlayer->GetUserID() ); 
	...
	if ( bBleed && pAttacker->IsPlayer() )
	{
		...
		if( info.GetDamageType() & (DMG_IGNITE | DMG_SONIC) )
		{
			bBleed = false;
		}
	}
...
void CTFPlayer::Event_KilledOther( CBaseEntity *pVictim, const CTakeDamageInfo &info )
{
	...
	if( pVictim->IsPlayer() )
	{
		...
		{
			//CBaseObject *pObj = dynamic_cast<CBaseObject*>( info.GetAttacker() );
...
void CTFPlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( info.GetAttacker() && info.GetAttacker()->IsPlayer() )
	{
		...
		// Setup for the achievement Trophy Kill. Done here due to deaths clearing conditions.
		if( pPlayerAttacker && pPlayerAttacker->IsPlayerClass( TF_CLASS_SNIPER ) 
			&& IsPlayerClass( TF_CLASS_HEAVYWEAPONS ) && m_Shared.InCond( TF_COND_LEG_DAMAGED ) )
		{
			CSingleUserRecipientFilter filter( pPlayerAttacker );
			UserMessageBegin( filter, "AchievementEvent" );
			WRITE_SHORT( ACHIEVEMENT_PF_NADE_TROPHY_KILL );
			MessageEnd();
		}
	}

	// Stop medigun self-healing just in case.
	{
		int iIndex = m_Shared.FindHealerIndex( this );
		if (iIndex != m_Shared.m_aHealers.InvalidIndex())
			m_Shared.StopHealing( this );
	}
...
#ifdef PF2_DLL
	bool bEMP = ( info.GetDamageType() & DMG_DISSOLVE ) != 0;
#endif
	...
	DropAmmoPack(bEMP, pPlayerAttacker);
	...
	// prevents a bug where if you die you can't pickup the flag again
	if ( HasTheFlag() )
	{
		DropFlag();
	}
	...
	// If the player had a primed grenade, throw it
	for (int i = 0; i < TF_PLAYER_GRENADE_COUNT; i++)
	{
		CTFWeaponBaseGrenade* pGrenade = (CTFWeaponBaseGrenade*)Weapon_OwnsThisID( m_PlayerClass.GetData()->m_aGrenades[i] );
		if (pGrenade)
		{
	...
	// Cyanide; Include grenades in the weapon count
	for( int iWeapon = 0; iWeapon < TF_PLAYER_WEAPON_COUNT + TF_PLAYER_GRENADE_COUNT; ++iWeapon )
	{
		CTFWeaponBase *pWeapon = ( CTFWeaponBase * )GetWeapon( iWeapon );
	...
	else if( !info.GetAttacker() || info.GetAttacker() == this || info.GetAttacker()->IsBSPModel() )
	{
	...
	if( bGib || bRagdoll )
	{
		CreateRagdollEntity( bGib, bBurning, bEMP, bOnGround, info.GetDamageCustom() );
	}

	// Don't overflow the value for this.
	m_iHealth = 0;

	// This is a super small nitpick to me but armor is set to 0 upon death
	SetArmorValue( 0 );
...
{
#ifdef PF2_DLL
	// 33.3% chance of playing
	if (!pf_deathanims_always.GetBool() && RandomInt( 1, 3 ) != 1)
		return false;
#else
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFPlayer::DropAmmoPack( bool bEMP, CBaseEntity* pInflictor )
{
	...
	CTFWeaponBase *pWeapon = (CTFWeaponBase*)GetWeapon( OBJ_DISPENSER );	
	...
	CTFAmmoPack *pAmmoPack = CTFAmmoPack::Create( vecPackOrigin, vecPackAngles, this, pszWorldModel, bEMP );
	Assert( pAmmoPack );
	if( pAmmoPack )
	{
		...
		{
			pAmmoPack->SetExplosionOwner(pInflictor);
		}
...
void CTFPlayer::TeamFortress_RemoveEverythingFromWorld( bool bSilent )
{
	TeamFortress_RemoveRockets();
	TeamFortress_RemovePipebombs();
#ifdef PF2_DLL
	TeamFortress_RemoveGrenades();
	TeamFortress_RemoveNails();
#endif
...
#ifdef PF2_DLL
	RemoveOwnedEnt( "tf_projectile_flame_rocket" );
#endif
}
...
void CTFPlayer::TeamFortress_RemovePipebombs( void )
{
	...
	{
#ifdef PF2_DLL
		RemoveOwnedEnt( "tf_projectile_pipe", TF_ENT_GRENADE );
#else
...
#ifdef PF2_DLL
//=========================================================================
// Removes the grenades
void CTFPlayer::TeamFortress_RemoveGrenades( void )
{
	RemoveOwnedEnt( "tf_weapon_grenade_caltrop_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_concussion_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_emp_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_gas_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_heal_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_mirv_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_mirv_bomb", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_nail_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_napalm_projectile", TF_ENT_GRENADE );
	RemoveOwnedEnt( "tf_weapon_grenade_normal_projectile", TF_ENT_GRENADE );
}

void CTFPlayer::TeamFortress_RemoveNails(void)
{
	RemoveOwnedEnt( "tf_projectile_nail", TF_ENT_NAIL );
	RemoveOwnedEnt( "tf_projectile_syringe", TF_ENT_NAIL );
	RemoveOwnedEnt( "tf_projectile_dart", TF_ENT_NAIL );
}
#endif
...
#ifdef PF2_DLL
void CTFPlayer::RemoveOwnedEnt( char *pEntName, unsigned int eEntType /*= TF_ENT_NORMAL*/ )
{
	...
	while( pEnt )
	{
		...
		if( !bOwner && eEntType > 0 )
		{
			if(eEntType == TF_ENT_GRENADE)
			{
			...
			else if(eEntType == TF_ENT_NAIL)
			{
				CTFBaseProjectile *pNail = dynamic_cast<CTFBaseProjectile*>(pEnt);
				Assert( pNail );
				if( pNail )
					bOwner = (pNail->GetScorer() == this);
			}
		}
...
void CTFPlayer::ForceRespawn( void )
{
	...
	if ( iDesiredClass == TF_CLASS_RANDOM )
	{
		...
		} while( iDesiredClass == GetPlayerClass()->GetClassIndex() || !TFGameRules()->CanJoinClass( this, iDesiredClass ));
	}
...
void CTFPlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
	{
		if( sv_cheats->GetBool() )
		{
			...
			TakeArmor( GetPlayerClass()->GetMaxArmor() );
#endif
...
void CTFPlayer::PainSound( const CTakeDamageInfo &info )
{
...
#ifdef PF2_DLL
	// Don't play falling pain sounds for concussion blasts
	...
	{
		EmitSound( "TFPlayer.Cough" );
		m_flNextPainSoundTime = gpGlobals->curtime + 1.0;
		return;
	}
	...
	// Cyanide; Want to play the fall damage sound with the normal hurt sound
	// Seal: We don't really need three sounds playing at once, 1. Beartrap closing 2. Normal pain sound 3. Fall damage sound. Way too noisey and distracting.
	/*if( info.GetDamageCustom() == TF_DMG_CUSTOM_CALTROP )
	{
...
void CTFPlayer::DeathSound( const CTakeDamageInfo &info )
{
	...
	// Cyanide; Want to play the fall damage sound with the normal death sound
	// Seal: We don't really need three sounds playing at once, 1. Beartrap closing 2. Normal death sound 3. Fall damage sound. Way too noisey and distracting.
/*	if( info.GetDamageCustom() == TF_DMG_CUSTOM_CALTROP )
	{
		CRecipientFilter filter;
		filter.AddRecipientsByPAS( GetAbsOrigin() );
...
{
	//forgot how long a napalm lasts
#define ACHIEVEMENT_BURN_TIME_WINDOW	6.0f
#define ACHIEVEMENT_BURN_VICTIMS	5
	...
	if( m_aBurnOtherTimes.Count() >= ACHIEVEMENT_BURN_VICTIMS )
	{
		CSingleUserRecipientFilter filter( this );
		UserMessageBegin( filter, "AchievementEvent" );
		WRITE_SHORT( ACHIEVEMENT_PF_NAPALM_BURN );
		MessageEnd();
	}
}
...
void CTFPlayer::GetOpposingTFTeamList(CUtlVector<CTFTeam *> *pTeamList, bool bGetAll /*= false*/)
{
	if( bGetAll )
	{
...
//-----------------------------------------------------------------------------
void CTFPlayer::CreateRagdollEntity( bool bGib, bool bBurning, bool bDisintegrate, bool bOnGround, int iDamageCustom )
{
	...
	if( pRagdoll )
	{
		...
		if( bDisintegrate )
		{
			pRagdoll->GetBaseAnimating()->Dissolve( "", gpGlobals->curtime, false, ENTITY_DISSOLVE_ELECTRICAL );
		}
	}
...
{
	if (!m_Shared.InCond( TF_COND_STEALTHED ) || !m_Shared.InCond( TF_COND_SMOKE_BOMB ))
		return;
...
#ifdef PF2_DLL
//-----------------------------------------------------------------------------
// Purpose: throw the flag
//-----------------------------------------------------------------------------
void CC_ThrowItem( void )
{
	if( pf_flag_allow_throwing.GetBool() )
	{
		CTFPlayer *pPlayer = ToTFPlayer( UTIL_GetCommandClient() );
		if( pPlayer )
		{
...
static ConCommand throwitem( "throwitem", CC_ThrowItem, "Throw the flag." );
#endif
...
void CTFPlayer::ModifyOrAppendCriteria( AI_CriteriaSet& criteriaSet )
{
	...
	if( pActiveWeapon )
	{
		...
		case TF_WPN_TYPE_PDA:
			...
			break;
		/*case TF_WPN_TYPE_SCOUTSHOTGUN:
			criteriaSet.AppendCriteria("weaponmode", "scoutshotgun");
			break;*/
...
{
	return (gpGlobals->curtime > m_flNextVoiceCommandTime) && IsAlive();
}

ConVar pf_allow_voice_spam( "pf_allow_voice_spam", "0", FCVAR_NOTIFY );
//-----------------------------------------------------------------------------
...
{
	float flNextVoiceTime = gpGlobals->curtime - m_flNextVoiceCommandTime;
	if( !pf_allow_voice_spam.GetBool() && flNextVoiceTime <= 5.0 ) {
		if( flNextVoiceTime < 1.0 ) {
			++m_iSpeakSpamCounter;
		}
	}
	else 
	...
	flNextVoiceTime = tf_max_voice_speak_delay.GetFloat();
	if( flSceneDuration < flNextVoiceTime ) {
		flNextVoiceTime = flSceneDuration;
	}
	m_flNextVoiceCommandTime = flNextVoiceTime + gpGlobals->curtime;
	if( m_iSpeakSpamCounter > 0 ) {
		m_flNextVoiceCommandTime += m_iSpeakSpamCounter * 0.5;
	}
}
...
bool CTFPlayer::WantsLagCompensationOnEntity( const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	...
	if( pPlayer->GetTeamNumber() == GetTeamNumber() )
	{
		...
		{
			// always do lag compensation with these weapons
			switch (GetActiveTFWeapon()->GetWeaponID() )
			{
	...
	if( !TFGameRules()->FriendlyFireMode() && pPlayer->GetTeamNumber() == GetTeamNumber() && !bDoLagComp )
		return false;
...
CON_COMMAND_F( getobjectname, "Get the object name.", FCVAR_CHEAT )
{
	const char* pEntityName = nullptr;
	int pEntityType = 0;
	CBaseEntity* pEntity = FindPickerEntity(UTIL_GetCommandClient());
	if ( pEntity )
	{
		pEntityName = pEntity->GetClassname();
		if ( V_strcmp( pEntityName, "obj_teleporter" ) == 0)
		{
			CBaseObject* pTeleporter = (CBaseObject*)pEntity;
			pEntityType = pTeleporter->GetObjectMode();
			Msg("The entity name is: %s, and the object type is: %d\n", pEntityName, pEntityType);
			return;
		}
		Msg("The entity name is: %s\n", pEntityName);
	}
	else
	{
		Msg("No entity found.\n");
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTFPlayer::PlayerIsDev( void )
{
...
void CTFPlayer::PowerplayThink( void )
{
	...
	{
		SetContextThink( &CTFPlayer::PowerplayThink, gpGlobals->curtime + RandomFloat( 2, 5 ), "TFPlayerLThink" );
	}
}
...
bool CTFPlayer::ShouldAnnounceAchievement( void )
{
	if( IsPlayerClass( TF_CLASS_SPY ) )
	{
		if( m_Shared.InCond( TF_COND_STEALTHED ) ||
#ifdef PF2_DLL
			m_Shared.InCond( TF_COND_SMOKE_BOMB ) ||
#endif
			m_Shared.InCond( TF_COND_DISGUISED ) ||
