...
public:
...
#ifdef PF2_DLL
	virtual int			TakeArmor( int iArmor );
#endif

#ifdef USE_INVENTORY
	int GetWeaponPreset( int iSlotNum );
	int GetWeaponPreset( int iClass, int iSlotNum );
	void HandleCommand_WeaponPresetNow( int iClass, int iSlotNum, int iPresetNum );
	bool HandleCommand_WeaponPreset( int iClass, int iSlotNum, int iPresetNum );
	virtual void		RegenerateRespawn();
#endif
	...
	// Team.
	void				ForceChangeTeam( int iTeamNum, bool bSilent = false );
	...
	// Utility.
	void				RemoveOwnedEnt( char *pEntName, unsigned int eEntType = TF_ENT_NORMAL );
	...
	void TeamFortress_RemoveGrenades();
	void TeamFortress_RemoveNails();
...
#ifdef PF2_DLL
	void GetOpposingTFTeamList( CUtlVector<CTFTeam *> *pTeamList, bool bGetAll = false );
#endif
	...
	void CreateRagdollEntity( bool bGib, bool bBurning, bool bDisintegrate, bool bOnGround, int iDamageCustom );
	void DestroyRagdoll( void );
	CNetworkHandle( CBaseEntity, m_hRagdoll );	// networked entity handle 
	virtual bool ShouldGib( const CTakeDamageInfo &info );

	// Dropping Ammo
	void DropAmmoPack( bool bEMP, CBaseEntity* pInflictor );
	...
	bool ShouldToggleSniperCharge( void ) { return m_bDoToggleSniperCharge; }
	void SetToggleSniperCharge( bool bTogglable ) { m_bDoToggleSniperCharge = bTogglable; }
	bool IsViewModelFlipped( void ) { return m_bFlipViewmodel; }
	void SetViewmodelFlipped( bool bFlipViewmodel ) { m_bFlipViewmodel = bFlipViewmodel; }
...
public: 
	bool m_bKamikazeByFrag;
	int m_iKamikazeFragEntIndex;
#endif
};
